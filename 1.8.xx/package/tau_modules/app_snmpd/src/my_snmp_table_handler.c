...
BOOL snmp_table_set_commit(SNMP_TABLE_T *pstTable, char *szOperator)
{
	...
	{
		if (!pstTable->pfTableValidFunc(szRecData, NULL))
		{
			agent_printf(LOG_ERR, "%s:snmp_table_set_commit pfTableValidFunc failed\n",
				SNMPAGENT_CONTROL_NAME);
            /* 2013-12-7 zch mod, 失败则return FALSE */
            #if 0
            /* BEGIN: Added by zoucaihong, 2013/10/17 */
            /*失败，旧数据覆盖新数据*/
    		memcpy(szRecData, szOriRecData, pstTable->ulTableRecLen*pstTable->ulMaxTableRecNum);
            /* END: Added by zoucaihong, 2013/10/17 */
            #endif
			return FALSE;
		}
	}
    /*成功，新数据覆盖旧数据*/
	memcpy(szOriRecData, szRecData, pstTable->ulTableRecLen*pstTable->ulMaxTableRecNum);
                           ...
                           UINT16   usColumnID)
{
	...
	{
		#if 1
		agent_printf(LOG_ERR, "snmp_table_set_column() column:%s read-only.\n",
			pstColumn->acColumnName);
		#else
		agent_printf(LOG_ERR, "snmp_table_set_column() table:%s column:%u read-only.\n",
			pstTable->acTableName,
		...
		{			
			#if 1
			agent_printf(LOG_ERR, "snmp_table_set_column() column:%s value changed.\n",
				pstColumn->acColumnName);
			#else
...
//    VOID            *pRecData = NULL;
	...
	#if 1
	agent_printf(LOG_DEBUG, "acTableName=%s,acColumnName=%s\n",pstTable->acTableName, pstColumn->acColumnName);
	#else
	agent_printf(LOG_DEBUG, "acTableName=%s,acColumnName=%u\n",pstTable->acTableName, pstColumn->usColumnID);
	#endif
...
//	UINT16 usColumnLen = 0;
	...
	{		
		...
		{
		    #if 0
		    printf("*(UINT32 *)(pValueAddr+pstTable->apstKeyColumns[k]->usOffSet :%d \n",(*(UINT32 
		    *)(pValueAddr+pstTable->apstKeyColumns[k]->usOffSet)));
		    #endif
...
BOOL snmp_table_check_row_integrity(SNMP_TABLE_T *pstTable, oid_t *row)
{
    ...
    //printf("ulOffset=%d row->subid_list_length=%d\n", ulOffset, row->subid_list_length);
	if(ulOffset == row->subid_list_length)
	{
		return TRUE;
	}
...
//	UINT32 n=0;
	...
	// column node
	//printf("request->oid_list[i].subid_list_length=%d ulOffset=%d\n", request->oid_list[i].subid_list_length, ulOffset);
...
endofmib:
	...
	memcpy(&response->value_list[response->value_list_length].oid,
		&request->oid_list[i], sizeof (request->oid_list[i]));
    /* BEGIN: Added by caihong, 2014/5/29 */
#if 0
	memcpy(&response->value_list[response->value_list_length].data,
    ...
    /* END: Added by caihong, 2014/5/29 */
