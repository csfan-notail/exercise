...
{	
    ...
    /*成功，新数据覆盖旧数据*/
    //printf("szRecData=%p\n", szRecData);
    //printf("szOriRecData=%p\n", szOriRecData);
    if(szOriRecData != NULL)
	    memcpy(szOriRecData, szRecData, pstTable->ulTableRecLen);
    else
	    agent_printf(LOG_ERR, "%s: snmp_scalar_set_commit szOriRecData null\n", SNMPAGENT_CONTROL_NAME);
...
int snmp_scalar_handler(SNMP_TABLE_T *pstTable, request_t *request, int i, response_t *response, client_t *client)
{
	...
	ulOffset = pstTable->ulOIDSize; //11
	//printf("ulOffset=%u\n", ulOffset);
	// column node
	...
	//snmp_log(LOG_INFO, "oid:%s", oid_ntoa(&(request->oid_list[i])));
	//printf("acTableName:%s ulColumnIndex:%u", pstTable->acTableName, ulColumnIndex);
    //printf("ulColumnIndex=%u pstTable->ulColumnNum=%u\n", ulColumnIndex, pstTable->ulColumnNum);
	//.0
	if(request->oid_list[i].subid_list_length>ulOffset)
	{
	...
	switch(request->type)
	{
		...
		case BER_TYPE_SNMP_SET:	
		    ...
		    //#endif
			#if 0
		    bRet = snmp_scalar_set_commit(pstTable, request->community);//////////////////////////////INIT填的值
    		if (!bRet)
    		{
    		    lprintf(LOG_ERR, "File:%s, Line:%d: snmp_scalar_set_commit failed\n",
    				__FILE__, __LINE__);
