...
extern int snmp_udp_send(unsigned char *pucServerIP, unsigned short usServerPort, unsigned int ulTimeout,
				  ...
				  unsigned int ulReqDataLen)
{
        ...
        ucReqDataType == ASN_TIMETICKS  )
	{
		if( ulReqDataLen != 4)
		{
            lprintf(LOG_ERR, "ulReqDataLen != 4   -- 492  fail");
            return -1;
		}
    ...
    {
        lprintf(LOG_ERR, "snmp_init_var: create oid var failed");
        return -1;
    }
	pstTLV->ulLen = ucOIDLen;
	DATA_DUP(pstTLV, aucOID, ucOIDLen);
                          ...
                          unsigned int ulMaxReqPDULen, unsigned int *pulReqPDULen)
{
    ...
    for(pstVar=pstPDU->pstVarList; pstVar; pstVar=pstVar->pstNext)
    {
	    ...
	    aucBuff[0]='\0';
	    pucValue2=pucValue;
	    for(i = 0; i < pstVar->stValue.ucLenNum; i++)
	    {
            sprintf(aucBuff,"%s%02X",aucBuff,pucValue2[i]);
	    }
	    for(j = 0; j < pstVar->stValue.ulLen; j++)
	    {
            sprintf(aucBuff,"%s%02X",aucBuff,pucValue2[i+j]);
	    }
	
	    aucBuff[0]='\0';
	    for(i = 0; i < pstVar->stValue.ucLenNum; i++)
	    {
            sprintf(aucBuff,"%s%02X",aucBuff,pstVar->stValue.aucLen[i]);
	    }
	    for(j = 0; j < pstVar->stValue.ulLen; j++)
	    {
            sprintf(aucBuff,"%s%02X",aucBuff,pstVar->stValue.pucValue[j]);
	    }
                          ...
                          unsigned int ulMaxReqPDULen, unsigned int *pulReqPDULen)
{
    ...
    for(pstVar=pstPDU->pstVarList; pstVar; pstVar=pstVar->pstNext)
    {
	    ...
	    agent_printf(LOG_MSG,"[%d]Len:%d,BUFF:%s",NodeNo,pstVar->stValue.ulLen,aucBuff);
	    aucBuff[0]='\0';
	    for(i = 0; i < pstVar->stValue.ucLenNum; i++)
	    {
            sprintf(aucBuff,"%s%02X",aucBuff,pstVar->stValue.aucLen[i]);
...
*****************************************************************************/
UINT32 snmp_get_dev_uptime(VOID)
{
...
* 功    能: 生成trap2操作报文，初始化pstPDU中的每一个TLV结构 stPDU、stVer	、stCommunity、stSnmpPDU、stReqID	、stErrStatu
* 参    数:
* 参数名称			输入/输出				描述
* 函数返回: 
* 说    明:
...
//	unsigned char		ucOIDLen = 0;
    ...
    {
        lprintf(LOG_ERR, "snmp_init_trap2_pdu: snmp_init_var of timestamp failed");
        return -1;
    }
                  ...
                  SNMP_TLV_T	*pstReqID)
{
    ...
    {    
	    ...
	    if (iRet < 0)
	    {
	        snmp_log(LOG_ERR, "sendto (%d)failed\n", iRet);
	        return -1;
	    }
...
/*************************************************************************
* 函数名称: snmp_send_trap_discover
* 功    能: send discover trap
* 参    数:
* 参数名称			输入/输出				描述
* 函数返回: 
* 说    明: 
...
int snmp_send_trap_discover(SNMP_SESSION_T *pstSession, SNMP_VAR_T *pstVarList)
{
	...
	if (!pstSession)
	{
        nms_log(LOG_ERR, "pstSession is null\n");
		return -1;
	}
    ...
    /*初始化stPDU中的每一个TLV结构 stPDU、stVer	、stCommunity、stSnmpPDU、
stReqID	、stErrStatu*/
    iRet = snmp_init_trap2_pdu(pstSession->acCommunity,&stPDU,ucTrapType);
    if (iRet < 0)
    {
        nms_log(LOG_ERR, "snmp_init_set_pdu failed\n");
        goto fail;
    }
    ...
    if (iRet < 0)
    {
        nms_log(LOG_ERR, "snmp_create_trap2_pdu failed\n");
		goto fail;
    }
 
    iRet = snmp_udp_send(pstSession->aucAgentIP, pstSession->usAgentPort, 
                         pstSession->ulTimeout, 
						 pucReqPDU, ulReqPDULen,
						 pucRspData, ulBufLen, &ulRecvMsgLen,
						 &(stPDU.stReqID));
    if (iRet < 0)
    {
        nms_log(LOG_ERR, "snmp_udp_send failed\n");
        goto fail;
    }
