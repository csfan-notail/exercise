...
extern UINT32       g_uiOfflineCount;
extern UINT32       g_uiOfflineFlag;
...
int snmp(client_t *client)
{
	...
	if (BER_TYPE_SNMP_RESPONSE == request.type)
	{
	    //dump_packet(&g_udp_client);

	    g_uiOfflineFlag = 2;

	    g_uiOfflineCount = 1;       /* clear offline count timer */
	...
	for(i=0;i<response.value_list_length;i++)
	{
		if(NULL != response.value_list[i].data.buffer)
		{
		     //printf("response.value_list[i].data.buffer=%s\n", response.value_list[i].data.buffer);
			    free(response.value_list[i].data.buffer);
		}
	}
