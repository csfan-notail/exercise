    ...
    if( LTE_OK == iRet)
   	{
		...
		sprintf(acTmpStr,"echo %d > /tmp/uci/iRssi",stLteInfo.stCellInfo.iRssi);   //rssi
		system(acTmpStr);
		usleep(500000);
		sprintf(acTmpStr,"echo %d > /tmp/uci/iRsrp",stLteInfo.stCellInfo.iRsrp);   //rsrp
		system(acTmpStr);
		usleep(500000);
		sprintf(acTmpStr,"echo %d > /tmp/uci/iRsrq",stLteInfo.stCellInfo.iRsrq);   //iRsrq
		system(acTmpStr);
		usleep(500000);	
		sprintf(acTmpStr,"echo %d > /tmp/uci/iSinr",stLteInfo.stCellInfo.iSinr);   //iSinr
		system(acTmpStr);
		usleep(500000);	
		sprintf(acTmpStr,"echo %s > /tmp/uci/acCellid",stLteInfo.stCellInfo.acCellId);   //cellid
		system(acTmpStr);
		usleep(500000);	
		sprintf(acTmpStr,"echo %s > /tmp/uci/acPhyCellid",stLteInfo.stCellInfo.acPhyCeillid);   //phycellid
		system(acTmpStr);
...
INT32 lte_get_simstatus()
{
   ...
   if(LTE_OK == iRet1)
   {
	 ...
	 sprintf(acTmpStr,"echo %d > /tmp/uci/enSimState",stSimState.enSimState);   //enSimState
	 system(acTmpStr);
   }
   else
   {
      sprintf(acTmpStr,"echo 0 > /tmp/uci/enSimState");   //enSimState
	  system(acTmpStr);
 ...
 CHAR acTmpStr[32] = {0};
  ...
  iRet2 = lte_iccid_get(&stICCID);  
    ...
    if( LTE_OK == iRet1 )
    {
        ...
        sprintf(acCmd,"echo %s > /tmp/uci/acIMSI",acTmpStr);
        system(acCmd);
    }
    
    else
    {
		sprintf(acCmd,"echo 0 > /tmp/uci/acIMSI");   //acIMSI
		system(acCmd);
		usleep(500000);
    ...
    if(  LTE_OK == iRet2 )
    {
        ...
        sprintf(acCmd,"echo %s > /tmp/uci/acICCID",acTmpStr);
		system(acCmd);
    }
    else
    {
		sprintf(acCmd,"echo 0 > /tmp/uci/acICCID");   //acIMSI
		system(acCmd);
		usleep(500000);
...
INT32 lte_get_pin()
{
 INT32 iRet = 0;
  ...
  if(iRet == LTE_OK)
  {
	...
	sprintf(acTmpStr,"echo %d > /tmp/uci/enState",stState.enState);   //enState
	system(acTmpStr);
  }
  else
  {
		sprintf(acTmpStr,"echo -1 > /tmp/uci/enState");   //enState
		system(acTmpStr);
...
INT32 lte_get_dial_status()
{
  ...
  {       
	...
	sprintf(acTmpStr,"echo %d > /tmp/uci/enLinkstate",stLinkState.enLinkstate);   //enLinkstate
	system(acTmpStr);
  }
  else
  {
    
	sprintf(acTmpStr,"echo 0 > /tmp/uci/enLinkstate");   //enLinkstate
	system(acTmpStr);
...
INT32 lte_get_operator()
{
  ...
  if(iRet == LTE_OK)
  {
	...
	sprintf(acTmpStr,"echo %s > /tmp/uci/acOperName",stOperInfo.acOperName);   //acOperName
	system(acTmpStr);
  }
  else
  {
   
	sprintf(acTmpStr,"echo 0 > /tmp/uci/acOperName");   //acOperName
	system(acTmpStr);
...
INT32 lte_get_access_network()
{
  ...
  if(iRet == LTE_OK)
  {
     ...
     {
        system("echo '4G' > /tmp/uci/enAccessNet");
     }
     else if(2 == stAccessNetMode.enAccessNet)
     {
        system("echo '3G' > /tmp/uci/enAccessNet");
     }
     else if(1 == stAccessNetMode.enAccessNet)
     {
        system("echo '2G' > /tmp/uci/enAccessNet");
     }
     
  }
  else
  {
   	system("echo 'N/A' > /tmp/uci/enAccessNet");
...
INT32 lte_get_imei()
{
  ...
  if(iRet == LTE_OK)
  {
    ...
    sprintf(acTmpStr,"echo %s > /tmp/uci/acIMEI",stImei.acImei);   //acOperName
	system(acTmpStr);
  }
  else
  {
    system("echo 'N/A' > /tmp/uci/acIMEI");
...
INT32 lte_get_lte_version()
{
  ...
  if(iRet == LTE_OK)
  {
    ...
    sprintf(acTmpStr,"echo %s > /tmp/uci/acVersion",stModuleType.acVersion);   //acOperName
	system(acTmpStr);
  }
  else
  {
    system("echo 'N/A' > /tmp/uci/acVersion");
...
INT32 lte_get_tac()
{
  ...
  if(iRet == LTE_OK)
  {
	 ...
	 sprintf(acTmpStr,"echo %s > /tmp/uci/acTac",stTac.acTac);   //acTac
	 system(acTmpStr);
  }
  else
  {
...
{	
	...
	if(iRet == LTE_OK)
	{
	    ...
	    sprintf(acTmpStr,"echo %d > /tmp/uci/iQosCid",stQosData.iQosCid);
	 	system(acTmpStr);
		usleep(500000);
		 sprintf(acTmpStr,"echo %d > /tmp/uci/iQosQci",stQosData.iQosQci);
	 	system(acTmpStr);
		usleep(500000);
		 sprintf(acTmpStr,"echo %d > /tmp/uci/iQosDlGbr",stQosData.iQosDlGbr);
	 	system(acTmpStr);
		 usleep(500000);
		sprintf(acTmpStr,"echo %d > /tmp/uci/iQosUlGbr",stQosData.iQosUlGbr);
	 	system(acTmpStr);
		usleep(500000);
		sprintf(acTmpStr,"echo %d > /tmp/uci/iQosDlMbr",stQosData.iQosDlMbr);
	 	system(acTmpStr);
		usleep(500000);
		 sprintf(acTmpStr,"echo %d > /tmp/uci/iQosUlMbr",stQosData.iQosUlMbr);
	 	system(acTmpStr);
	}
	else
    {
    	system("echo 0 > /tmp/uci/iQosCid");
    	usleep(500000);
    	system("echo 0 > /tmp/uci/iQosQci");
    	usleep(500000);
    	system("echo 0 > /tmp/uci/iQosDlGbr");
    	usleep(500000);
    	system("echo 0 > /tmp/uci/iQosUlGbr");
    	usleep(500000);
    	system("echo 0 > /tmp/uci/iQosDlMbr");
    	usleep(500000);
    	system("echo 0 > /tmp/uci/iQosUlMbr");
...
INT32 main(INT32 iArgc, CHAR **apcArgv[])
{
	...
	{ 
		...
		sleep(1);
		lte_get_qos_data();
		printf("[%25s|%30s:%5d] iGetinfoTime=%d \n",__FILE__,__FUNCTION__,__LINE__,iGetinfoTime);
		sleep(iGetinfoTime);
	} while ( 1 );
