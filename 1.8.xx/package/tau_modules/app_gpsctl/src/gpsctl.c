...
int get_parameter(int argc,char** args)
{
	int iIndex = 0 ;
    int iChar ;
	while((iChar = getopt_long(argc, args, "i:d:t:l:h", long_options, &iIndex)) != EOF)
    {
    	switch (iChar)
    	{
			case 'i':
			...
			case 'd':
    		{
				...
				sscanf(g_acGpsDate, "%4d-%2d-%2d",&tm_temp.tm_year, &tm_temp.tm_mon, &tm_temp.tm_mday);
			...
			case 't':
    		{
				...
				sscanf(g_acGpsTime, "%2d:%2d:%2d",&tm_temp.tm_hour, &tm_temp.tm_min, &tm_temp.tm_sec);
    			break;
    		}
			case 'l':
    		{
                /* «∑Ò µ ±…Ë÷√date ˝æ›*/
                g_iGpsSet = atoi(optarg);
				GPS_LOG(LOG_INFO,"[gps]  g_iGpsdeal %d \n",g_iGpsSet);
    			break;
    		}
    		case 'h':
...
{   
	...
	if( 1 == g_iGpsSet )
	{
		sprintf(acBuff,"date -s '%04d-%02d-%02d %02d:%02d:%02d'",pstGPS->D.year,pstGPS->D.month,pstGPS->D.day,pstGPS->D.hour,pstGPS->D.minute,pstGPS->D.second);
		GPS_LOG(LOG_INFO,"cmd =  %s \n",acBuff);  
		system(acBuff);
	}
}  
...
static unsigned long endian_reversal(unsigned long value)
{
	return (value & 0x000000FFU) << 24 | (value & 0x0000FF00U) << 8 | (value & 0x00FF0000U) >> 8 | (value & 0xFF000000U) >> 24;
}

int writecom_gps(int iComFd)
{


	

	//printf("%s ------- %s\n",g_acGpsDate,g_acGpsTime);

	//sscanf(g_acGpsDate, "%[^:]:%[^:]:%[^:]", acYear,acMonth,acDay);
	//sscanf(g_acGpsTime, "%[^:]:%[^:]:%[^:]", acHour,acMinute,acSecond);
	tm_temp.tm_hour = tm_temp.tm_hour - 8;//gpsœ‡≤Ó8∏ˆ–° ±
    printf("%04d-%02d-%02d %02d:%02d:%02d\n",tm_temp.tm_year,tm_temp.tm_mon,tm_temp.tm_mday,
    					tm_temp.tm_hour, tm_temp.tm_min, tm_temp.tm_sec);
    printf("%04x-%02x-%02x %02x:%02x:%02x\n",tm_temp.tm_year,tm_temp.tm_mon,tm_temp.tm_mday,
    					tm_temp.tm_hour, tm_temp.tm_min, tm_temp.tm_sec);

	tm_temp.tm_year=endian_reversal(tm_temp.tm_year);
	tm_temp.tm_mon=endian_reversal(tm_temp.tm_mon);
	tm_temp.tm_mday=endian_reversal(tm_temp.tm_mday);
	tm_temp.tm_hour=endian_reversal(tm_temp.tm_hour);
	tm_temp.tm_min=endian_reversal(tm_temp.tm_min);
	tm_temp.tm_sec=endian_reversal(tm_temp.tm_sec);
		
	unsigned char acData[30] = {0xF1, 0xD9, 0x0B, 0x02, 0x16, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00, 0x0B, 0x0B, 0x0B, 0x0B, 0x0B, 0x00, 0x00, 0x00, 0xE1, 0x07, 0x42, 0xE1};
	int iSize = sizeof(acData);
	int iRetCounter = 0;
	int i = 0;

	memcpy(&acData[26],&tm_temp.tm_year,2);
	memcpy(&acData[18],&tm_temp.tm_mon,1);
	memcpy(&acData[19],&tm_temp.tm_mday,1);
	memcpy(&acData[20],&tm_temp.tm_hour,1);
	memcpy(&acData[21],&tm_temp.tm_min,1);
	memcpy(&acData[22],&tm_temp.tm_sec,1);


	unsigned char CheckSUM1=0,CheckSUM2 = 0;
	for(i=2;i<28;i++)
	{
		printf("acData[%02d]=0x%02x",i,acData[i]);
		printf("\t");
		CheckSUM1 += acData[i];
		CheckSUM2 += CheckSUM1;
		printf("CheckSUM1=0x%02x",CheckSUM1);
		printf("\t");
		printf("CheckSUM2=0x%02x",CheckSUM2);
		printf("\n");
	}
	memcpy(&acData[28],&CheckSUM1,1);
	memcpy(&acData[29],&CheckSUM2,1);
	...
	for (i=0;i<iSize;i++)
	{		
		iRetCounter = write(iComFd,&acData[i],1);
		printf("acData[%02d]=0x%02x after writing ,return data length is %d,and counter is %d\n",i,acData[i],iRetCounter,i);
	}
...
int write_gps_com(void)
{
	...
	while ( 1 ) 
	{
		...
		if ((iComFd = open("/dev/ttyUSB20",O_RDWR | O_NOCTTY)) < 0) 
		{
			usleep(25*1000);
			perror("open");
			continue;
	...
	if (init_serial(iComFd,115200, 8, 'N', 1)< 0)
	{
		close(iComFd);
		return -1;
	}
