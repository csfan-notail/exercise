...
LTE_RET_E huawei4G_connect(void *pvData, int iDataLen)
{
    ...
    else
    {
        if(NULL != strstr(pstApn->acApn, "+"))
        {
            ...
            {
	            sleep(3);
	            system("ifconfig usb1 down up");
        	}
        
	        if(NULL != strstr(acPDPStack, "v4"))
	        {
		        system("ifconfig usb1 0.0.0.0");
                ...
                {
                    system_call("udhcpc -i usb1 -s /etc/udhcpc.script &");
                }
                else
                {
                    //system("dhclient -p 68 usb1 -cf /var/db/dhclient.conf &");
                } 
            }
        }
        else
