...
athrs_ar8033_reg_init(void *arg)
{
    ...
    custom_reset_phy();
    printf("%s: Resetting PHY...\n",__func__ );
    //udelay(8 * 1000000);

    /* 
    Disable EEE operation
      MMD7 ¡ª EEE advertisement register: Device Address = 7; Offset = 0x3C 
      MMD access control register: Offset: 0x0D
      MMD access data register: Offset: 0x0E
    */
    int value;
    phy_reg_write(0x0, 0x0, 0xd, 0x7);
    phy_reg_write(0x0, 0x0, 0xe, 0x3c);
    phy_reg_write(0x0, 0x0, 0xd, 0x4007);
    value = phy_reg_read(0x0, 0x0, 0xe);
    value &= ~(0x3 << 1);
    phy_reg_write(0x0, 0x0, 0xe, value);
    printf("%s: EEE operation disabled.\n", __func__);        
    
	phy_reg_write(0x0, 0x0, 0x1f, 0x8101);
	printf("%s: Done %x \n",__func__, phy_reg_read(0x0, 0x0, 0x1f));
#else
