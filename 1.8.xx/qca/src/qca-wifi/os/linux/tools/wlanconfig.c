...
#define OTHER_SSID "Others   \0"
    ...
    if(set_atp.info_cnt)
    {
        ...
        for (i =0; i < set_atp.info_cnt; i++)
        {
            ...
            if((!strncmp(ssid,set_atp.atf_info[i].ssid, ssid_length ) && (strlen(set_atp.atf_info[i].ssid) == ssid_length)) || !strncmp("Others   ",set_atp.atf_info[i].ssid, strlen(OTHER_SSID)))
                {
