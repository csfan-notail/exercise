...
static void ieee80211_node_iter_fairq_algo(void *arg, struct ieee80211_node *ni)
{
    ...
    if(ni->ni_atfdata_logged < ATF_DATA_LOG_SIZE)
    {
        ...
        if(ni->ni_atfindex)
        {
            ...
            if( (node_unusedtokens <= ni->tx_tokens) && (ni->tx_tokens) )
            {
                ni->ni_unusedtokenpercent[ni->ni_atfindex -1 ] = ((node_unusedtokens/ni->tx_tokens) * 100);
            }
            else
            {
    ...
    /* tx_tokens will be zero until atfcfg_timer updates atf_units */
    if( (node_unusedtokens <= (ni->tx_tokens - ni->ni_contributedtokens)) && (ni->tx_tokens) )
    {
        ni->ni_unusedtokenpercent[ node_index ] =
                ((node_unusedtokens * 100)/ (ni->tx_tokens - ni->ni_contributedtokens));
    }
    else
    {
