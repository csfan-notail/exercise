...
enable_qcawifi() {
	...
	for vif in $vifs; do
		...
		config_get encode "$vif" encode
		
		config_get ssid "$vif" ssid
                [ -n "$ssid" ] && { 
                # conver encode
                	if [ "$encode" = "GB2312" ];then
                		echo $ssid >/tmp/ssid_encode
                		iconv -c -s -f UTF-8 -t GB2312 /tmp/ssid_encode >/tmp/ssid_encode_dst
                		ssid=`cat /tmp/ssid_encode_dst`
                	fi
		} && {
...
config wifi-iface   ath${devidx}0
