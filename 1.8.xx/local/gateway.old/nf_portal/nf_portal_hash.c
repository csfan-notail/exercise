...
LOCAL struct hlist_head *nf_portal_sta_hash_fn(UINT8 *pucMac,UINT32 uiIp)
{
    UINT32 uiIndex;
    ...
    uiIndex = nf_portal_mac_ip_hash(pucMac,uiIp);
...
NF_PORTAL_STA_ENTRY_T *nf_portal_get_sta_from_htable(UINT8 *pucMac,UINT32 uiIp)
{
    ...
    hlist_for_each_safe(pos, n,nf_portal_sta_hash_fn(pucMac,uiIp))
    {
        pstEntry = hlist_entry(pos, NF_PORTAL_STA_ENTRY_T, stStaHList);

        if ((0 == memcmp(pstEntry->stSta.aucMac,pucMac,ETH_ALEN)) && (uiIp == pstEntry->stSta.uiIp))
        {
...
NF_PORTAL_STA_ENTRY_T *nf_portal_add_sta_to_htable(UINT8 *pucMac,UINT32 uiIp)
{
    ...
    pstEntry = nf_portal_get_sta_from_htable(pucMac,uiIp);
    ...
    pstHead = nf_portal_sta_hash_fn(pucMac,uiIp);
