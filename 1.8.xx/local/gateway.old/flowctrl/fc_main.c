...
AP_ERROR_CODE_E fc_hook_destroy(void)
{
    ...
    //nf_unregister_hooks(fc_hooks,2);
    return AP_E_NONE;
}
...
#if 0    
    /*获取radio id 和wlan id*/
    iArgNum = sscanf(szVapName, "ap%u_%u", &stSta.iRadioId, &stSta.iWlanId);
    if (iArgNum != 2)
    {
        return AP_E_DEFAULT_FAIL;
    }
    
    FC_LOG(FC_LOG_INFO,"STA (ap%d_%d)"MAC_FMT" auto learn: %d:%d/%d:%d\n",
                    stSta.iRadioId, 
                    stSta.iWlanId,
                    MAC_ARG(stSta.acMac), 
