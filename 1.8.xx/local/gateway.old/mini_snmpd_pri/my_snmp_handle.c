...
BOOL snmp_table_index_valid(UINT32 ulTableID, UINT32 ulTableRecLen, VOID *pKey)
{
    ...
    if ( (TABLE_ID_AP_STAT_ONLINEUSERTABLE == ulTableID)
        || (TABLE_ID_AP_STAT_APINFO == ulTableID)
        || (TABLE_ID_AP_STAT_VERSION == ulTableID)
        || (TABLE_ID_AP_STAT_AP_STA == ulTableID)
        )
    {
        if ( 0 == memcmp(pKey, aucMac, 6) )
        {
            return FALSE;
        }
        else
...
SNMP_TABLE_T *snmp_get_next_table(SNMP_TABLE_T *pstPrevTable)
{
	...
	if (NULL == pstPrevTable ||
        /* BEGIN: Modified by zoucaihong, 2014/7/4 */
        /* TABLE ID不是按顺序排列，中间会插入有特殊含义的TABLE ID，
        改为使用MAX_TABLE_NUM来做TABLE ID的合法性检查 */
		//pstPrevTable->ulTableID>=g_pstTableInfo->ulTableNum+2
		pstPrevTable->ulTableID>=MAX_TABLE_NUM
        /* END: Modified by zoucaihong, 2014/7/4 */
		)
	{
		return NULL;
	}
...
int snmp_row_build(SNMP_TABLE_T *pstTable, UINT32 ulColumnIndex, CHAR  *pValueAddr, oid_t *row)
{
	...
	for(j=0;j<ulKeyColumnNum;j++)
	{
		...
		switch(ucKeyColumnType)
		{
			...
			case TYPE_MAC:
                //lprintf(LOG_INFO, "pValueAddr["MAC_FMT"].", MAC_ARG(pValueAddr));
				for(k=0;k<6;k++)
				{
					row->subid_list[ulOffset+k] = (UINT8)pValueAddr[k];
                    //lprintf(LOG_INFO, "row->subid_list[%d]:%u.", ulOffset+k, row->subid_list[ulOffset+k]);
				}
				ulOffset+= 6;
				pValueAddr += usKeyColumnLen;
				break;
...
{	
    ...
    //lprintf(LOG_INFO, "subid_list_length:%d", value->oid.subid_list_length);
	/* feng jing add*/
...
BOOL snmp_convert_byte_short(VOID *pInValue, UINT32 ulInLen, UINT16 *pusTmp)
{
    UINT16  usTmp=0;
    ...
    lprintf(LOG_ERR, "usTmp:%d\n", usTmp);
    *pusTmp = usTmp;
    return TRUE;
}

BOOL snmp_convert_byte_long(VOID *pInValue, UINT32 ulInLen, UINT32 *pulTmp)
{
    UINT32  ulTmp=0;
    UINT16  usTmp=0;
    ...
    lprintf(LOG_ERR, "ulTmp:%d\n", ulTmp);
    *pulTmp = ulTmp;
    return TRUE;
}

BOOL snmp_convert_byte_int(VOID *pInValue, UINT32 ulInLen, INT32 *piTmp)
{
    INT32  ilTmp=0;
    INT16  isTmp=0;
                     ...
                     VOID       *pOutValue)
{
    ...
    /*for ( ulLoop = 0 ; ulLoop < ulInLen+1 ; ulLoop++ )
    {
        agent_printf(LOG_ERR, "pInValue:%x ulInLen:%d.",pInValue[ulLoop], ulInLen);
    }*/
    ...
    switch(ulColumnType)
    {
        ...
        case TYPE_SHORT:
            #ifdef GW_X86
            snmp_convert_byte_short(pInValue,ulInLen, &usTmp);
            #endif
            /* BEGIN: Modified by 李文, 2012/10/12   PN:CPE远程维护 
            鉴于SNMP协议，对于数字类型的值，是变长，则不应该与定值做等于比较*/
            ...
            if (ulInLen <= sizeof(UINT16) && ulColumnLen == sizeof(UINT16))
            {
                ...
                /* END:   Modified by 李文, 2012/10/12   PN:CPE远程维护 */
                #ifdef GW_X86
                memcpy(pOutValue,&usTmp,ulInLen);
                #else
		...
		case TYPE_COUNTER32:
            #ifdef GW_X86
            snmp_convert_byte_long(pInValue,ulInLen, &ulTmp);
            #endif
            /* BEGIN: Modified by 李文, 2012/10/12   PN:CPE远程维护 
            鉴于SNMP协议，对于数字类型的值，是变长，则不应该与定值做等于比较*/
            ...
            if (ulInLen <= sizeof(UINT32) && ulColumnLen == sizeof(UINT32))
            {
                ...
                /* END:   Modified by 李文, 2012/10/12   PN:CPE远程维护 */
                #ifdef GW_X86
                memcpy(pOutValue,&ulTmp,sizeof(UINT32));
                #else
                memcpy(pOutValue,pInValue,ulInLen);
                #endif
                /* BEGIN: Added by 李文, 2012/10/12   PN:CPE远程维护 
		...
		case TYPE_INT:
            #ifdef GW_X86
            snmp_convert_byte_int(pInValue, ulInLen, &iTmp);
            #endif
            /* BEGIN: Modified by 李文, 2012/10/12   PN:CPE远程维护 
            鉴于SNMP协议，对于数字类型的值，是变长，则不应该与定值做等于比较*/
			...
			if (ulInLen <= sizeof(INT32) && ulColumnLen == sizeof(INT32))
			{
                ...
                /* END:   Modified by 李文, 2012/10/12   PN:CPE远程维护 */
                #ifdef GW_X86
                memcpy(pOutValue,&iTmp,sizeof(INT32));
                #else
                memcpy(pOutValue,pInValue,ulInLen);
                #endif
                /* BEGIN: Added by 李文, 2012/10/12   PN:CPE远程维护 
...
int handle_snmp_getnext(request_t *request, response_t *response, client_t *client)
{
	...
	{		
		...
		if (pos == -1) {
			...
			if (request->version == SNMP_VERSION_1) {
                /* BEGIN: Modified by zoucaihong, 2012/12/21 
                snmp v1支持get-next操作*/
				//goto nosuchname;
				goto endofmib;
                /* END: Modified by zoucaihong, 2012/12/21 */
			} else if (response->value_list_length < MAX_NR_VALUES) {
				goto endofmib;
			} else {
