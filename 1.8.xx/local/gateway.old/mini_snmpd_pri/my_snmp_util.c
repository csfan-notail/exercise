...
BOOL agent_convert_time(UINT32 ulTime, CHAR *szTime)
{
	...
	if (NULL == szTime)
	{
		agent_printf(LOG_ERR, "ac_agent_convert_time:szTime is 0\n");
		return FALSE;
	}
	...
	if (time == NULL)
	{
		agent_printf(LOG_ERR, "ac_agent_convert_time:ulTime:%u localtime() return NULL\n",
			ulTime);
		return FALSE;
	}
...
{	
    ...
    lprintf(LOG_INFO, "snmp_get_table_record ulTableID:%u.",ulTableID);
	switch(ulTableID)
 	{
 	    #if 0
		...
		case TABLE_ID_AP_CONFIG_VAPBASICTABLE:
            agent_printf(LOG_ERR, "%s:snmp_get_table_record ac_agent_get_apvapbasicsetup_config start ulTableID %d\n",
					SNMPAGENT_CONTROL_NAME,ulTableID);
        ...
        case TABLE_ID_AP_CONFIG_USER:
            pucData = (UINT8 *)&(g_pstSnmpAgentData->stUserConfig);
            break;
        case TABLE_ID_AP_CONFIG_GWSSID:
            pucData = (UINT8 *)&(g_pstSnmpAgentData->stGwSsidConfig);
            break;
        case TABLE_ID_AP_CONFIG_AUTH:
            pucData = (UINT8 *)&(g_pstSnmpAgentData->stAuthConfig);
            break;
        case TABLE_ID_AP_CONFIG_BATCH_BWLIST:
            pucData = (UINT8 *)&(g_pstSnmpAgentData->stBatchBwlistConfig);
            break;
        case TABLE_ID_AP_CONFIG_DOAMINFILTER:
            pucData = (UINT8 *)&(g_pstSnmpAgentData->stUrlBwConfig);
            break;
        case TABLE_ID_AP_CONFIG_USER_SPEED:
            pucData = (UINT8 *)&(g_pstSnmpAgentData->stUserSpeedConfig);
            break;
        case TABLE_ID_AP_CONFIG_WHITE_USER_SPEED:
            pucData = (UINT8 *)&(g_pstSnmpAgentData->stWhiteUserSpeedConfig);
            break;
        case TABLE_ID_AP_CONFIG_USER_LOGOUT:
            pucData = (UINT8 *)&(g_pstSnmpAgentData->stUserLogoutConfig);
            break;
        case TABLE_ID_AP_CONFIG_GET_SECRET_KEY:
            pucData = (UINT8 *)&(g_pstSnmpAgentData->stGetSecretKeyConfig);
            break;
		...
		case TABLE_ID_AP_STAT_SYS:
			...
			pucData = (UINT8 *)(&g_pstSnmpAgentData->stSysStat);
			break;
#if 0
...
#endif
        /* BEGIN: Added by zoucaihong, 2015/3/25 */
		case TABLE_ID_AP_STAT_ONLINEUSERTABLE:
            //ac_agent_get_onlineuser();
			pucData = (UINT8 *)(g_pstSnmpAgentData->pstOnlineUserInfo);
			break;
		case TABLE_ID_AP_STAT_APINFO:
            //ac_agent_get_apinfo();
			pucData = (UINT8 *)(g_pstSnmpAgentData->pstApInfo);
			break;
		case TABLE_ID_AP_STAT_VERSION:
            //ac_agent_get_apversioninfo();
			pucData = (UINT8 *)(g_pstSnmpAgentData->pstApVersionInfo);
			break;
		case TABLE_ID_AP_STAT_AP_STA:
            //ac_agent_get_apstainfo();
			pucData = (UINT8 *)(g_pstSnmpAgentData->pstGwApStaInfo);
			break;
        /* END: Added by zoucaihong, 2015/3/25 */
        ...
        /* BEGIN: Added by zoucaihong, 2015/3/25 */
		case TABLE_ID_AP_OPER_GWAP:
			pucData = (UINT8 *)&(g_pstSnmpAgentData->stGwApOper);
			break;
        /* END: Added by zoucaihong, 2015/3/25 */
...
VOID get_cpu_endian()
{
	...
	if(*(char *)&x == 1)
	{
		...
		g_ulEndian = SNMP_LITTLE_ENDIAN;
	}
	else
	{
	    lprintf(LOG_INFO, "big-endian"); 
		//g_ulEndian = BIG_ENDIAN;
		g_ulEndian = SNMP_BIG_ENDIAN;
	}
}
/*****************************************************************************
...
BOOL dev_get_cms_recvtime(UINT8 *pucFromIp)
{
    ...
    lprintf(LOG_INFO, "cmsIp("IP_FMT") fromIp("IP_FMT").", 
        IP_ARG(aucCmsIp), IP_ARG(pucFromIp));
...
BOOL app_get_cms_ip(UINT8 * pucSessionIp)
{
    ...
    UINT32  ulLoop = 0;
	FILE    *pfData = NULL;	
    CHAR    *szFileName= DHCP_OPTION43_CONF;
	CHAR	*pucPos = NULL;
    CHAR    *szTemp = NULL;
    CHAR    acBuf[128] = {0};
    UINT32  i = 0;
        ...
        && '\0' != acTrapDesIp[0] )
    {
        ...
        {
            aucTrapIp[ulLoop] = aulIp[ulLoop];
        }
    }
    /* 通过DHCP Option43获得CMS IP 
     * Dhclient 会生产一个配置文件ipcfg.conf，optiont43字段存放在GBCOM_accontroller中
     */
    else
    {
        pfData = fopen (szFileName, "rb");
        ...
        while ( fgets(acBuf, sizeof(acBuf)-1, pfData) )
        {
        	...
        	// 查询GBCOM_accontroller
        	pucPos = strstr(acBuf, "GBCOM_accontroller");
        	if (NULL != pucPos)
        	{
            	sscanf(acBuf, "GBCOM_accontroller="IP_FMT, &aulIp[0], &aulIp[1], &aulIp[2], &aulIp[3]);
                for ( ulLoop = 0 ; ulLoop <4 ; ulLoop++ )
                {
                    aucTrapIp[ulLoop] = aulIp[ulLoop];
                }
    ...
    memcpy(pucSessionIp, aucTrapIp, 4);
...
Options:
    ...
    */
    ...
    sprintf(acCmd,"busybox ftpget -u %s -p %s -P %u %u.%u.%u.%u %s %s",
        ...
        pstFileTrans->acFileName);	
#endif
    agent_printf(LOG_ERR,"Func:%s acCmd:%s begin.\n",__func__, acCmd);
    if(0 != system(acCmd))
    {
    ...
    */
	...
	while(1)
	{
		...
		if( NULL != pcPos )
		{
            ...
            /* 上次内核统计数据 */
            ulLastDevOctets = *pulIn%0x7FFFFFFF;
            /* 上次mib统计数据 */
            ulIfOctets = *pulIn;
            /* 内核出现2G翻转 */
            ...
            /* 上次内核统计数据 */
            ulLastDevOctets = *pulOut%0x7FFFFFFF;
            /* 上次mib统计数据 */
            ulIfOctets = *pulOut;
            /* 内核出现2G翻转 */
...
BOOL string_macs_to_array(CHAR *pcMacList, AP_MAC_LIST_T *pstMac)
{
    ...
    {
        ulFlag = 1;
        if ( NULL != (pos=strchr(pMac, ',')) )
        {
            *pos = '\0';
            string_mac_tools(pMac, pstMac->stMacList[ulMacNum].aucMac);
            lprintf(LOG_INFO, "ap mac:"MAC_FMT".\n",MAC_ARG(pstMac->stMacList[ulMacNum].aucMac));
            if ( 0 == memcmp(pstMac->stMacList[ulMacNum].aucMac, aucInvalidMac, 6) )
            {
    ...
    pstMac->ulNum = ulMacNum;
    lprintf(LOG_INFO, "mac num:%d.\n", ulMacNum);
    return TRUE;
}
...
BOOL multi_ssid_convert(CHAR *pcSsidList, AP_SSID_LIST_T *pstSsid)
{
    ...
    UINT8 aucMac[6] = {0};
    UINT8 aucInvalidMac[6] = {0};
    UINT32 ulStringLen = 0;

    pTmp = pcSsidList;
    while('\0' != *pTmp)
    {
        ulFlag = 1;
        if ( NULL != (pos=strchr(pTmp, ',')) )
        {
            *pos = '\0';
            strcpy(pstSsid->stSsidList[ulNum].acSsid, pTmp);
            lprintf(LOG_INFO, "ap ssid:%s.\n",pstSsid->stSsidList[ulNum].acSsid);
            ulStringLen = strlen(pstSsid->stSsidList[ulNum].acSsid);
            lprintf(LOG_INFO, "ulStringLen:%d.\n",ulStringLen);
            if ( 0 == ulStringLen )
            {
                lprintf(LOG_ERR, "ap ssid is invalid.\n");
                return FALSE;
            }
            ulNum++;
            pTmp += (ulStringLen+1);
            lprintf(LOG_INFO, "pTmp:%s.\n",pTmp);
            continue;
        }
        break;
    }
    ...
    {
        strcpy(pstSsid->stSsidList[ulNum].acSsid, pTmp);
        lprintf(LOG_ERR, "ap ssid:%s.\n",pstSsid->stSsidList[ulNum].acSsid);
        if ( 0 == strlen(pstSsid->stSsidList[ulNum].acSsid) )
        {
            lprintf(LOG_ERR, "ap ssid is invalid.\n");
            return FALSE;
        }
...
int snmp_add_ap_by_mac(UINT8 *pucMac)
{
    AP_HASH_T *pstAp;
    struct hlist_node *p;
    ...
    index = mac_hashfn(pucMac, AP_HASH_SIZE);
    ...
    /*AP未找到，加入AP*/
    pstAp = (AP_HASH_T*)malloc(sizeof(AP_HASH_T));
    memset(pstAp, 0, sizeof(AP_HASH_T));
    memcpy(pstAp->mac, pucMac, 6);

    hlist_add_head(&(pstAp->mac_Hlist), head);
    AP_LIST_UNLOCK();

    agent_printf(LOG_INFO, "ap-sta:  ap("MAC_FMT") is added", MAC_ARG(pstAp->mac));

    return 0;	
}

int snmp_del_ap_by_mac(UINT8 *pucMac)
{
    AP_HASH_T *pstAp;
    ...
    hlist_for_each_safe(p, n, head)
    {
        ...
        if (!memcmp(pstAp->mac, pucMac, 6))
        {
            ...
            agent_printf(LOG_INFO, "del AP ("MAC_FMT")", MAC_ARG(pstAp->mac));
            free(pstAp);
            break;
        }
    }
...
int update_ap_info_by_mac(unsigned char *mac, SNMP_AP_INFO_T *pstApInfo)
{
    AP_HASH_T *pstAp;
    struct hlist_node *p;
    struct hlist_head *head;
    ...
    hlist_for_each( p, head )
    {
        ...
        {
            memcpy(&pstAp->stApDetailInfo, pstApInfo , sizeof(SNMP_AP_INFO_T));
            lprintf(LOG_INFO, "ap("MAC_FMT")'s info changes ok!",MAC_ARG(pstAp->mac));
            lprintf(LOG_INFO, "snmp-apinfo mac:("MAC_FMT") ip("IP_FMT")!",
                MAC_ARG(pstAp->stApDetailInfo.acMac),
                IP_ARG(pstAp->stApDetailInfo.acIp));
    ...
    lprintf(LOG_INFO, "AP MAC:"MAC_FMT" "
        ...
        MAC_ARG(pstAp->stApDetailInfo.acMac),
        IP_ARG(pstAp->stApDetailInfo.acIp),
        pstAp->stApDetailInfo.acfap_sn,
        pstAp->stApDetailInfo.acDeviceType,
        pstAp->stApDetailInfo.acLocation,
        pstAp->stApDetailInfo.acVendor,
        pstAp->stApDetailInfo.iOnline,
        pstAp->stApDetailInfo.cur_link,
        pstAp->stApDetailInfo.total_link,
        pstAp->stApDetailInfo.iCha2,
        pstAp->stApDetailInfo.iCha5,
        pstAp->stApDetailInfo.iRoam,
        pstAp->stApDetailInfo.total_up,
        pstAp->stApDetailInfo.total_down,
        pstAp->stApDetailInfo.iRunStatus,
        pstAp->stApDetailInfo.runtimelong,
        pstAp->stApDetailInfo.iSsidNum,
        pstAp->stApDetailInfo.acMemo,
        pstAp->stApDetailInfo.acCpu_status,
        pstAp->stApDetailInfo.softversion
        );
    UINT32 ulLoop = 0;
    for ( ulLoop = 0 ; ulLoop < pstAp->stApDetailInfo.iSsidNum ; ulLoop++ )
    {
        if ( pstAp->stApDetailInfo.iSsidNum >= 16 )
        {
            break;
        }
        lprintf(LOG_INFO, "ap-ssid-info: ssidName[%d]:%s.",
            ulLoop,
            pstAp->stApDetailInfo.ssid[ulLoop].acSsid);
    }
    hlist_add_head(&(pstAp->mac_Hlist), head);
#endif    
    AP_LIST_UNLOCK();
...
int update_sta_info_by_ap_mac(unsigned char *mac, UINT32 ulStaNum, SNMP_STA_INFO_T *pstStaInfo)
{
    AP_HASH_T *pstAp;
    struct hlist_node *p;
    struct hlist_head *head;
    ...
    hlist_for_each( p, head )
    {
        ...
        if (!memcmp(pstAp->mac, mac, 6))
        {
            pstAp->ulStaNum = ulStaNum;
            for ( ulLoop = 0 ; ulLoop < ulStaNum ; ulLoop++ )
            {
                memcpy(&pstAp->astStaInfo[ulLoop], &pstStaInfo[ulLoop], sizeof(SNMP_STA_INFO_T));
                lprintf(LOG_INFO, "sta-info(%d): staMac("MAC_FMT") staIp("IP_FMT").",
                    ulLoop,
                    MAC_ARG(pstAp->astStaInfo[ulLoop].aucStaMac),
                    IP_ARG(pstAp->astStaInfo[ulLoop].aucIp)
                    );
            }
...
BOOL agent_update_apinfo(AP_STAT_APINFO_ENTRY_T *pstApInfo, UINT32 *pulNum)
{
    AP_HASH_T *pstAp;
    struct hlist_node *p;
    struct hlist_head *head;
    ...
    if ( NULL == pstApInfo )
    {
        lprintf(LOG_ERR, "pstApInfo is null.\n");
        return FALSE;
    }

	AP_LIST_LOCK();
	for(ulLoop=0; ulLoop<AP_HASH_SIZE; ++ulLoop)
    {
        hlist_for_each(p, &g_ap_HList[ulLoop])
        {
			pstAp = hlist_entry(p, AP_HASH_T, mac_Hlist);
            if ( NULL != pstAp )
            {
                lprintf(LOG_INFO, "ap-info: apMac("MAC_FMT")", MAC_ARG(pstAp->mac));
                memcpy(pstApInfo->aucApMac, pstAp->stApDetailInfo.acMac, 6);
                pstApInfo->ulApNo = ulRecIndex+1;
                memcpy(pstApInfo->aucApIp, pstAp->stApDetailInfo.acIp, 4);
                pstApInfo->ulCurLink = pstAp->stApDetailInfo.cur_link;
                pstApInfo->ulTotalLink = pstAp->stApDetailInfo.total_link;
                pstApInfo->ulRoad = pstAp->stApDetailInfo.iCha2;
                pstApInfo->ulTotalUp = pstAp->stApDetailInfo.total_up;
                pstApInfo->ulTotalDw = pstAp->stApDetailInfo.total_down;
                strcpy(pstApInfo->acApMemo, pstAp->stApDetailInfo.acMemo);
                pstApInfo->ulApStatus = pstAp->stApDetailInfo.iRunStatus;
                strcpy(pstApInfo->acApCpuStatus, pstAp->stApDetailInfo.acCpu_status);
                pstApInfo->ulApRuntimeLong = pstAp->stApDetailInfo.runtimelong;
                strcpy(pstApInfo->acApFirstSsid, pstAp->stApDetailInfo.ssid[0].acSsid);
                strcpy(pstApInfo->acApSecondSsid, pstAp->stApDetailInfo.ssid[1].acSsid);
                ulRecIndex++;
                lprintf(LOG_INFO, "ap-info: apMac("MAC_FMT") apIp("IP_FMT") curlink:%d totallink:%d cha2:%d tUp:%d tDw:%d.\n",
                    MAC_ARG(pstApInfo->aucApMac),
                    IP_ARG(pstApInfo->aucApIp),
                    pstApInfo->ulCurLink,pstApInfo->ulTotalLink,pstApInfo->ulRoad,
                    pstApInfo->ulTotalUp,pstApInfo->ulTotalDw
                    );
                pstApInfo++;
            }
		}
...
BOOL agent_update_apversioninfo( AP_STAT_VERSION_ENTRY_T *pstApVerInfo )
{
    AP_HASH_T *pstAp;
    struct hlist_node *p;
    struct hlist_head *head;
    ...
    if ( NULL == pstApVerInfo )
    {
        lprintf(LOG_ERR, "pstApVerInfo is null.\n");
        return FALSE;
    }
	...
	for(ulLoop=0; ulLoop<AP_HASH_SIZE; ++ulLoop)
    {
        hlist_for_each(p, &g_ap_HList[ulLoop])
        {
            ...
            if ( NULL != pstAp )
            {
                lprintf(LOG_INFO, "ap-version-info: apMac("MAC_FMT")", MAC_ARG(pstAp->mac));
                memcpy(pstApVerInfo->aucApMac, pstAp->stApDetailInfo.acMac, 6);
                pstApVerInfo->ulApNo = ulRecIndex+1;
                strcpy(pstApVerInfo->acCurVer, pstAp->stApDetailInfo.softversion);
                strcpy(pstApVerInfo->acNewVer, pstAp->stApDetailInfo.softversion);
                pstApVerInfo->ulApStatus = pstAp->stApDetailInfo.iRunStatus;
                ulRecIndex++;
                lprintf(LOG_INFO, "ap-version-info: apMac("MAC_FMT") CurVer(%s) NewVer(%s).",
                    MAC_ARG(pstApVerInfo->aucApMac),
                    pstApVerInfo->acCurVer,
                    pstApVerInfo->acNewVer
                    );
                pstApVerInfo++;
            }
		}
...
BOOL agent_update_apstainfo(AP_STAT_AP_STA_ENTRY_T *pstApStaInfo)
{
    AP_HASH_T *pstAp;
    struct hlist_node *p;
    struct hlist_head *head;
    ...
    {
        lprintf(LOG_ERR, "pstApStaInfo is null.\n");
        return FALSE;
    }
	...
	for(ulLoop=0; ulLoop<AP_HASH_SIZE; ++ulLoop)
    {
        hlist_for_each(p, &g_ap_HList[ulLoop])
        {
            ...
            if ( NULL != pstAp )
            {
                lprintf(LOG_INFO, "sta-info: apMac("MAC_FMT")", MAC_ARG(pstAp->mac));
                for ( ulLoopSta = 0 ; ulLoopSta < pstAp->ulStaNum; ulLoopSta++ )
                {
                    memcpy(pstApStaInfo[ulRecIndex].aucApMac, pstAp->stApDetailInfo.acMac, 6);
                    memcpy(pstApStaInfo[ulRecIndex].aucStaMac, pstAp->astStaInfo[ulLoopSta].aucStaMac, 6);
                    pstApStaInfo[ulRecIndex].ulStaNo = ulRecIndex+1;
                    sprintf(pstApStaInfo[ulRecIndex].acStaName, "用户%d", pstApStaInfo[ulRecIndex].ulStaNo);
                    memcpy(pstApStaInfo[ulRecIndex].aucStaIp, pstAp->astStaInfo[ulLoopSta].aucIp, 4);
                    pstApStaInfo[ulRecIndex].ulRssi = pstAp->astStaInfo[ulLoopSta].rssi;
                    pstApStaInfo[ulRecIndex].ulConnectTime = pstAp->astStaInfo[ulLoopSta].connect_time;
                    pstApStaInfo[ulRecIndex].ulStaTotalUp = pstAp->astStaInfo[ulLoopSta].total_up;
                    pstApStaInfo[ulRecIndex].ulStaTotalDw = pstAp->astStaInfo[ulLoopSta].total_down;
                    lprintf(LOG_INFO, "sta-info: loop(%d) apMac("MAC_FMT") staMac("MAC_FMT") staIp("IP_FMT").",
                        ulRecIndex,
                        MAC_ARG(pstApStaInfo[ulRecIndex].aucApMac),
                        MAC_ARG(pstApStaInfo[ulRecIndex].aucStaMac),
                        IP_ARG(pstApStaInfo[ulRecIndex].aucStaIp)
                        );
                    ulRecIndex++;
                }
            }
