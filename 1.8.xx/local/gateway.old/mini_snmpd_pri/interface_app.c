...
BOOL app_get_sys_devinfo(AP_CONFIG_SYS_DEVINFO_T *pstDevInfo)
{
    ...
    */
    ...
    pstDevInfo->ulRunState = 1;
    strcpy(pstDevInfo->acPartner, "00000000");
    strcpy(pstDevInfo->acLineName, ETH0);
    get_oem_company(pstDevInfo->acManufacture);
    get_customer_prosn(pstDevInfo->acSn);
    string_mac_tools(pstDevInfo->acSn, aucSn);
    memset(pstDevInfo->acSn, 0, sizeof(pstDevInfo->acSn));
    sprintf(pstDevInfo->acSn, SN_FMT, MAC_ARG(aucSn));

    agent_printf(LOG_INFO, "the dev MAC: "MAC_FMT" the dev IP:"IP_FMT" the dev uptime:%d"
        " sysModel:%d InterModel:%s sysModelName:%s ulDevType:%d acSysName:%s acManufacture:%s acSn:%s.\n",
...
BOOL app_get_running_version(CHAR *pRVersion)
{
    ...
    // get addr mask information
    sprintf(command, "vflag list");
    if(NULL != (pf = popen(command, "r")))
    {
        memset(buf, 0, sizeof(buf));
        while(NULL != fgets(buf, sizeof(buf)-1, pf))
        {
            buf[sizeof(buf)-1] = '\0';

            /* 去除行尾空格 */
            for (i=strlen(buf)-1; i >= 0 && IS_EMPTY_CHAR(buf[i]); i--)
            {
                buf[i] = '\0';
            }

            /* 跳过空格 */
            for(szTemp = buf; IS_EMPTY_CHAR(szTemp[0]); szTemp ++);
            ...
            start = strstr(buf, "Version  : ");
            if (NULL != start)
            {
                strcpy(pRVersion, start+11);
                strcat(pRVersion, "-");
            }
            start = strstr(buf, "Revision : ");
            if (NULL != start)
            {
                strcat(pRVersion, start+11);
                break;
            }            
        }
        pclose(pf);
    }    
    agent_printf(LOG_INFO, "softerversion = [%s]\n", pRVersion);
    return;
#else
    FILE *pf;
    char *start = NULL;
    ...
    if(NULL != (pf = popen(command, "r")))
    {
        ...
        while(NULL != fgets(buf, sizeof(buf), pf))
        {
            ...
            start = strstr(buf, "running version");
            if (NULL != start)
            {
            #ifdef AP93_OAM
                fgets(buf, sizeof(buf), pf);//ap93
                strcpy(pRVersion, buf);       
            #else
                strcpy(pRVersion, buf+17);    //ta40 8M 16M ta98
            #endif
                break;
            }
        }
...
UINT32 app_get_swinfo(AP_CONFIG_SYS_SWINFO_T *pstSwInfo)
{
    int i=0;
    char buf[32]={0};
    nvram_get(buf, NVRAM_SOFT_VERSION, NVRAM_SIZE);
    for (i=strlen(buf)-1; i >= 0 && IS_EMPTY_CHAR(buf[i]); i--)
    {
        buf[i] = '\0';
    }
    strcpy(pstSwInfo->acOemVersion, buf);
...
BOOL app_get_hdinfo(AP_CONFIG_SYS_HWINFO_T *pstHdInfo)
{
    ...
    /* 内存大小 */
    pstHdInfo->ulMemSize = 64;

    /* 磁盘大小 */
    pstHdInfo->ulDiskSize = 16;
    
    /* 获取硬件版本号 */
    nvram_get(pstHdInfo->acHardwareVersion, NVRAM_HARD_VERSION, NVRAM_SIZE);
    sprintf(pstHdInfo->acCpuInfo, "533MHz");
