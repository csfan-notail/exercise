...
BOOL agent_send_dev_offline_trap()
{
    ...
    {
        agent_printf(LOG_ERR, "agent_send_alarm_trap ulAlarmCode:%u fail.", ALARM_ID_DEV_OFFLINE);
        return FALSE;
    }
    agent_printf(LOG_INFO, "agent_send_alarm_trap ulAlarmCode:%u success.", ALARM_ID_DEV_OFFLINE);

	return TRUE;
}
/* END: Added by zoucaihong, 2014/6/27 */
/* BEGIN: Added by zoucaihong, 2015/3/25 */
BOOL agent_send_gw_basicinfo_trap()
{
    SNMP_VAR_T              *pstVarList = NULL;
	SNMP_SESSION_T          stSnmpSession;
    int                     ret = -1; 
	...
	/* binding variable */
	SNMP_INIT_TRAP_VAR(pstVarList, oid_ap_alarm_gw_basicinfo_trap, sizeof(oid_ap_alarm_gw_basicinfo_trap));
    
	SNMP_ADD_TRAP_VAR(pstVarList, oid_dev_hotId,    TYPE_CHAR,  g_pstSnmpAgentData->pstSysDevInfo->acHotId);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_dev_sysName,  TYPE_CHAR,  g_pstSnmpAgentData->pstSysDevInfo->acSysName);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_gw_partner,   TYPE_CHAR,  g_pstSnmpAgentData->pstSysDevInfo->acPartner);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_dev_ip,       TYPE_IP,    g_pstSnmpAgentData->pstSysDevInfo->aucIpAddress);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_dev_software_oemver, TYPE_CHAR,  g_pstSnmpAgentData->pstSysSwInfo->acOemVersion);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_dev_mac,      TYPE_MAC,   g_aucDevMac);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_gw_line,      TYPE_CHAR,  g_pstSnmpAgentData->pstSysDevInfo->acLineName);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_gw_ipmode,    TYPE_LONG,  g_pstSnmpAgentData->pstSysIpConfig[0].ulIpMode);
    ...
    {
    	agent_printf(LOG_DEBUG,"agent_send_gw_basicinfo_trap success!!");
    }

	return TRUE;
}
BOOL agent_send_gw_zcbasicinfo_trap()
{
    SNMP_VAR_T              *pstVarList = NULL;
	SNMP_SESSION_T          stSnmpSession;
    int                     ret = -1; 
	...
	/* binding variable */
	SNMP_INIT_TRAP_VAR(pstVarList, oid_ap_alarm_gw_zcbasicinfo_trap, sizeof(oid_ap_alarm_gw_zcbasicinfo_trap));
    
	SNMP_ADD_TRAP_VAR(pstVarList, oid_dev_mac,          TYPE_MAC,   g_aucDevMac);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_dev_manufacture,  TYPE_CHAR,  g_pstSnmpAgentData->pstSysDevInfo->acManufacture);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_dev_sn,           TYPE_CHAR,  g_pstSnmpAgentData->pstSysDevInfo->acSn);
    if(g_pstSnmpAgentData->pstApVapBasicConfig)
        SNMP_ADD_TRAP_VAR(pstVarList, oid_dev_ssid,         TYPE_CHAR,  g_pstSnmpAgentData->pstApVapBasicConfig[0].acSsid);
	...
	if(-1 == ret) 
    {
    	agent_printf(LOG_ERR,"agent_send_gw_zcbasicinfo_trap fail");
        return FALSE; 
    }
    else
    {
    	agent_printf(LOG_DEBUG,"agent_send_gw_zcbasicinfo_trap success!!");
    }

	return TRUE;
}
BOOL agent_send_gw_dyninfo_trap()
{
    SNMP_VAR_T              *pstVarList = NULL;
	SNMP_SESSION_T          stSnmpSession;
    int                     ret = -1; 
	...
	/* binding variable */
	SNMP_INIT_TRAP_VAR(pstVarList, oid_ap_alarm_gw_dyninfo_trap, sizeof(oid_ap_alarm_gw_dyninfo_trap));
    
	SNMP_ADD_TRAP_VAR(pstVarList, oid_dev_hotId,        TYPE_CHAR,   g_pstSnmpAgentData->pstSysDevInfo->acHotId);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_gw_runstate,      TYPE_LONG,    g_pstSnmpAgentData->pstSysDevInfo->ulRunState);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_gw_apnum,         TYPE_LONG,  g_pstSnmpAgentData->stSysStat.ulApNum);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_gw_aponlinenum,   TYPE_LONG,  g_pstSnmpAgentData->stSysStat.ulApOnlineNum);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_gw_apofflinenum,  TYPE_LONG,  g_pstSnmpAgentData->stSysStat.ulApOfflineNum);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_gw_onlineusernum, TYPE_LONG,  g_pstSnmpAgentData->stSysStat.ulOnlineUserNum);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_gw_feelusernum,   TYPE_LONG,  g_pstSnmpAgentData->stSysStat.ulFeelUserNum);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_gw_cpuusage,      TYPE_LONG,  g_pstSnmpAgentData->stSysStat.ulCPURTUsage);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_gw_memusage,      TYPE_LONG,  g_pstSnmpAgentData->stSysStat.ulMemRTUsage);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_dev_uptime,       TYPE_LONG,  g_pstSnmpAgentData->pstSysDevInfo->ulSysUpTime);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_dev_heart_period, TYPE_LONG,  g_pstSnmpAgentData->pstSysDevInfo->ulHeartBeatPeriod);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_gw_totalup,       TYPE_LONGLONG,  g_pstSnmpAgentData->stSysStat.ulTotalUp);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_gw_totaldw,       TYPE_LONGLONG,  g_pstSnmpAgentData->stSysStat.ulTotalDw);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_gw_upspeed,       TYPE_LONG,  g_pstSnmpAgentData->stSysStat.ulUpSpeed);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_gw_dwspeed,       TYPE_LONG,  g_pstSnmpAgentData->stSysStat.ulDwSpeed);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_gw_totalband,     TYPE_LONG,  g_pstSnmpAgentData->stSysStat.ulTotalWideBand);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_gw_bandused,      TYPE_LONG,  g_pstSnmpAgentData->stSysStat.ulWidebandPerused);
	...
	if(-1 == ret) 
    {
    	agent_printf(LOG_ERR,"agent_send_gw_dyninfo_trap fail");
        return FALSE; 
    }
    else
    {
    	agent_printf(LOG_DEBUG,"agent_send_gw_dyninfo_trap success!!");
    }

	return TRUE;
}
BOOL agent_send_gw_userinfo_trap(AP_STAT_ONLINEUSER_ENTRY_T *pstUserInfo)
{
    SNMP_VAR_T              *pstVarList = NULL;
	SNMP_SESSION_T          stSnmpSession;
    int                     ret = -1; 
	...
	/* binding variable */
	SNMP_INIT_TRAP_VAR(pstVarList, oid_ap_alarm_userinfo_trap, sizeof(oid_ap_alarm_userinfo_trap));
    
	SNMP_ADD_TRAP_VAR(pstVarList, oid_dev_mac,          TYPE_MAC,   g_aucDevMac);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_user_mac,         TYPE_MAC,   pstUserInfo->aucUserMac);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_user_ip,          TYPE_IP,    pstUserInfo->aucUserIp);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_user_ssid,        TYPE_CHAR,  pstUserInfo->acUserSsid);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_user_logintime,   TYPE_CHAR,  pstUserInfo->acUserLoginTime);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_user_logouttime,  TYPE_CHAR,  pstUserInfo->acUserLogoutTime);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_user_logouttype,  TYPE_CHAR,  pstUserInfo->acLogoutType);
	...
	if(-1 == ret) 
    {
    	agent_printf(LOG_ERR,"agent_send_gw_userinfo_trap fail");
        return FALSE; 
    }
    else
    {
    	agent_printf(LOG_INFO,"agent_send_gw_userinfo_trap success!!");
    }

	return TRUE;
}
BOOL agent_send_gw_clientinfo_trap(AP_STAT_AP_STA_ENTRY_T *pstClientInfo)
{
    SNMP_VAR_T              *pstVarList = NULL;
	SNMP_SESSION_T          stSnmpSession;
    int                     ret = -1; 
	...
	/* binding variable */
	SNMP_INIT_TRAP_VAR(pstVarList, oid_ap_alarm_clientinfo_trap, sizeof(oid_ap_alarm_clientinfo_trap));
    
	SNMP_ADD_TRAP_VAR(pstVarList, oid_dev_mac,          TYPE_MAC,   g_aucDevMac);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_client_mac,       TYPE_MAC,   pstClientInfo->aucStaMac);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_client_ip,        TYPE_IP,    pstClientInfo->aucStaIp);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_client_ssid,      TYPE_CHAR,  pstClientInfo->acClientSsid);
	SNMP_ADD_TRAP_VAR(pstVarList, oid_client_logintime, TYPE_CHAR,  pstClientInfo->acLoginTime);
	...
	if(-1 == ret) 
    {
    	agent_printf(LOG_ERR,"agent_send_gw_clientinfo_trap fail");
        return FALSE; 
    }
    else
    {
    	agent_printf(LOG_DEBUG,"agent_send_gw_clientinfo_trap success!!");
    }
