...
{	
    ...
    /* BEGIN: Added by zoucaihong, 2015/4/1 */
    ap_hash_init();
    /* END: Added by zoucaihong, 2015/4/1 */
	...
	for(;;)
	{
        ...
        if ( ulTicks%59 == 0 )
        {
            ...
            else
            {
                ...
                ac_agent_get_apsysstat();
                
    			OsSemaphoreV(gtSemStat, 1);
            }
            snmp_notify_wifidog_get_authcfg();
        }
...
BOOL agent_check_dev_state(VOID)
{
    ...
    {
        agent_printf(LOG_ERR, "g_enDevState:%d g_ulDisTicks:%d g_ulRecvTimeout:%d.",
            g_enDevState, g_ulDisTicks, g_ulRecvTimeout);
        agent_dev_state_change(DEV_STATE_DISCOVER);
    }
    else if((DEV_STATE_RUNNING == g_enDevState)&&
        (0 == (g_ulHeartTicks%ulHearBeatPeriod)))/* 发送心跳 */
    {
        agent_printf(LOG_ERR, "func:%s g_enDevState:%d is valid.\n",
            __func__, g_enDevState);
    ...
    /* BEGIN: Added by zoucaihong, 2015/3/25 */
        ...
        (0 == (g_ulTicks%(60*5))))/* 发送gwDynamicInfoTrap，上报网关动态信息 */
    {
...
int main(int argc, char *argv[])
{
    ...
    #endif
    /* BEGIN: Added by zoucaihong, 2014/12/3 */
    snmp_aglink_client_init();
    /* END: Added by zoucaihong, 2014/12/3 */
