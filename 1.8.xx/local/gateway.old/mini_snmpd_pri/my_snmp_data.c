...
//#endif
    ...
    /* 贝联需求 */
    bRet &= init_snmp_ap_config_domain_filter(pstTable++);
	if(FALSE == bRet)	{		return FALSE;	}
    bRet &= init_snmp_ap_config_user(pstTable++);
	if(FALSE == bRet)	{		return FALSE;	}
    bRet &= init_snmp_ap_config_gwssid(pstTable++);
	if(FALSE == bRet)	{		return FALSE;	}
    bRet &= init_snmp_ap_config_auth(pstTable++);
	if(FALSE == bRet)	{		return FALSE;	}
    bRet &= init_snmp_ap_config_batch_bwlist(pstTable++);
	if(FALSE == bRet)	{		return FALSE;	}
    /* 中传需求 */
    bRet &= init_snmp_ap_config_user_speed(pstTable++);
	if(FALSE == bRet)	{		return FALSE;	}
    bRet &= init_snmp_ap_config_white_user_speed(pstTable++);
	if(FALSE == bRet)	{		return FALSE;	}
    bRet &= init_snmp_ap_config_user_logout(pstTable++);
	if(FALSE == bRet)	{		return FALSE;	}
    bRet &= init_snmp_ap_config_get_secret_key(pstTable++);
	if(FALSE == bRet)	{		return FALSE;	}
...
#endif
    /* BEGIN: Added by zoucaihong, 2015/3/25 */
	bRet &= init_snmp_ap_stat_onlineuser_table(pstTable++, TABLE_MAXRECNUM_USER);
	if(FALSE == bRet)	{		return FALSE;	}
	bRet &= init_snmp_ap_stat_ap_table(pstTable++, TABLE_MAXRECNUM_AP);
	if(FALSE == bRet)	{		return FALSE;	}
	bRet &= init_snmp_ap_stat_version_table(pstTable++, TABLE_MAXRECNUM_AP);
	if(FALSE == bRet)	{		return FALSE;	}
	bRet &= init_snmp_ap_stat_ap_sta_table(pstTable++, TABLE_MAXRECNUM_USER);
	if(FALSE == bRet)	{		return FALSE;	}
    /* END: Added by zoucaihong, 2015/3/25 */
    ...
    /* BEGIN: Added by zoucaihong, 2015/3/25 */
	bRet &= init_snmp_ap_oper_gwap(pstTable++);
	if(FALSE == bRet)	{		return FALSE;	}
    /* END: Added by zoucaihong, 2015/3/25 */
