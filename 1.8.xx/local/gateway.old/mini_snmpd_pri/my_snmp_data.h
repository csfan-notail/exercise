...
typedef enum
{
    AP_STOPRECOVER_NORMAL=0,
    AP_STOPRECOVER_STOP,
    AP_STOPRECOVER_RECOVER
}AP_STOP_RECOVER_E;
...
#ifndef SNMP_LITTLE_ENDIAN
#define  SNMP_LITTLE_ENDIAN  ((UINT32)1)
#define  SNMP_BIG_ENDIAN     ((UINT32)2)
#endif
...
/* 网关对AP的操作 */
typedef enum{
    GW_AP_OPSTATE_NORMAL=0,
    GW_AP_OPSTATE_REBOOT=1,
    GW_AP_OPSTATE_UPGRADE=2,
    GW_AP_OPSTATE_UPDATE=3
}GW_AP_OPSTATE_E;

#define SNMPAGENT_USER_LOGIN    ((UINT32)1)
#define SNMPAGENT_USER_LOGOUT   ((UINT32)2)

#define DEV_DISCOVER_PERIOD      60//second
#define DEV_CMS_TIMEOUT          60*3//second
...
#define PERIOD_DYNAMICINFO  2*60

#define STATUS_NORMAL   0   /* 不做任何操作 */
#define STATUS_MODIFY   3   /* 修改记录 */
#define STATUS_ADD      1   /* 增加记录 */
#define STATUS_DELETE   2   /* 删除记录 */
...
#define TABLE_MAXRECNUM_AP              ((UINT32)512)
#define TABLE_MAXRECNUM_USER            ((UINT32)1024)
...
#define TABLE_NAME_AP_CONFIG_DOMAINFILTER       "domain-filter-list"
/* BEGIN: Added by zoucaihong, 2015/3/24 */
#define TABLE_NAME_AP_CONFIG_USER               "userConfig"
#define TABLE_NAME_AP_CONFIG_GWSSID             "gwSsidConfig"
#define TABLE_NAME_AP_CONFIG_AUTH               "authConfig"
#define TABLE_NAME_AP_CONFIG_BATCH_BWLIST       "batchBwListConfig"
/* END: Added by zoucaihong, 2015/3/24 */
#define TABLE_NAME_AP_CONFIG_USER_SPEED         "userSpeedConfig"
#define TABLE_NAME_AP_CONFIG_WHITE_USER_SPEED   "whiteUserSpeedConfig"
#define TABLE_NAME_AP_CONFIG_USER_LOGOUT        "userLogoutConfig"
#define TABLE_NAME_AP_CONFIG_GET_SECRET_KEY     "getSecretKeyConfig"
/*统计表*/
...
#endif
/* BEGIN: Added by zoucaihong, 2015/3/24 */
#define TABLE_NAME_AP_STAT_ONLINEUSERTABLE      "onlineUserStatTable"
#define TABLE_NAME_AP_STAT_APINFO               "apinfoTable"
#define TABLE_NAME_AP_STAT_VERSION              "apVersionTable"
#define TABLE_NAME_AP_STAT_AP_STA               "gwApStaTable"
/* END: Added by zoucaihong, 2015/3/24 */
/* BEGIN: Added by zoucaihong, 2014/5/14 */
...
/* END: Added by zoucaihong, 2014/5/14 */
/* BEGIN: Added by zoucaihong, 2015/3/24 */
#define TABLE_NAME_AP_OPER_GWAP                 "gwApOper"
/* END: Added by zoucaihong, 2015/3/24 */
...
typedef struct 
{
    ...
    /* BEGIN: Added by zoucaihong, 2015/3/24 */
    UINT32  ulRunState;         /* AP运行状态 */
    CHAR    acAuthCode;         /* 注册码，鹏博士项目用 */
    UINT32  ulAuthState;        /* 注册状态， 鹏博士项目用 */
    CHAR    acPartner[32];      /* 合作商ID ，贝联项目需求*/
    /* END: Added by zoucaihong, 2015/3/24 */
    CHAR    acLineName[32];     /* 线路名称，如wan1，贝联项目需求 */
    CHAR    acManufacture[32];  /* 设备厂商 */
    CHAR    acSn[32];           /* 设备序列号 */
...
typedef struct 
{
    ...
    UINT32  ulMemFree;      /* 鹏博士项目需求 */
    UINT32  ulApLoad;       /* 鹏博士项目需求 */
    /* 以下为贝联项目需求 */
    UINT32  ulApNum;
    UINT32  ulApOnlineNum;
    UINT32  ulApOfflineNum;
    UINT32  ulOnlineUserNum;
    UINT32  ulFeelUserNum;
    UINT64  ulTotalUp;
    UINT64  ulTotalDw;
    UINT32  ulUpSpeed;
    UINT32  ulDwSpeed;
    UINT32  ulTotalWideBand;
    UINT32  ulWidebandPerused;
}AP_STAT_SYS_T;
...
typedef struct 
{
    ...
    UINT8  aucStaDevType[32];   /*连线用戶终端设备类型*/
...
typedef struct
{
    ...
    /* END: Added by zoucaihong, 2014/7/15 */
    UINT32  ulSysStopRecover;   /* 中传项目需求，stop(1),recover(2) */
}AP_OPER_SYSOPER_T;
...
typedef struct
{
    UINT32  ulAction;       /* login(1),logout(2) */
    UINT8   aucMac[6];
    UINT8   aucResv[2];
    UINT32  ulAuthType;     /* 认证类型,可能值为1-9 用户密码为1,优惠券为2,固定密码为3,手 机为4,腾讯QQ为5,新浪微博为6,无密码 为7,微信为8,支付宝为9 */
    CHAR    acUserId[64];   /* 用户ID */
    UINT32  ulTimeLimit;    /* 本次认证上线可用时长，单位min，0-10000，设置为0时表示不限时长 */
    UINT32  ulCommit;       /* normal(0), commit(1) */
}AP_CONFIG_USER_T;

typedef struct
{
    CHAR    acGwSsid[512];                  /* 路由器名称,为数组 */
}AP_CONFIG_GWSSID_T;

typedef struct
{
    UINT32  ulSwitch;       /* 无线认证启用开关,1为开启,0为关闭 */
    UINT32  ulWebAuth;      /* Web认证启用开关,1为开启,0为关闭 */
    UINT32  ulOffline;      /* 自动下线,超过时间无流量则强制其下线,0 为无限制,单位秒 */
    UINT32  ulReverify;     /* 重新认证,上网超过指定时须重新认证,0 为无限制,单位分钟 */
    CHAR    acFreeMac[512]; /* 免认证 MAC 地址，数组 */
}AP_CONFIG_AUTH_T;

typedef struct
{
    UINT32  ulListType;     /* 名单类型，white(1), black(2) */
    CHAR    acMacList[512]; /* MAC地址 */
    UINT32  ulStatus;       /* 操作类型，normal(0), modify(1), add(2), del(3) */
}AP_CONFIG_BATCH_BWLIST_T;

/* URL黑白名单 */
typedef struct
{
    UINT32  ulIndex;            /* SSID编号,指定ssid上的用户黑白名单,编号范围:1~6,同SSID设置的编号(鹏博士使用) */
    CHAR    acDomain[512];      /* 域名或IP地址 */
    UINT32  ulType;             /* 1:黑名单,2:白名单 */
    UINT32  ulStatus;           /* 用于记录的创建、删除 */
}AP_CONFIG_DOMAIN_FILTER_T;
...
typedef struct
{
    ...
    UINT32  ulDwloadMax;
    UINT32  ulCommit;
}AP_CONFIG_USER_SPEED_T;

/* 用户下线配置，中传需求 */
typedef struct
{
    UINT32  ulLogoutStatus;     /* 是否开启,off(0),on(1) */
    UINT32  ulTimeCondition;    /* 下线时间门限 */
    UINT32  ulFlowCondition;    /* 下线流量门限，KB */
    UINT32  ulCommit;           /* 配置提交 */
}AP_CONFIG_USER_LOGOUT_T;

/* 密钥获取，中传需求 */
typedef struct
{
    UINT32  ulInterval;         /* 密钥中传服务器密钥的时间间隔，单位秒 */
}AP_CONFIG_GET_SECRET_KEY_T;

/* 用户表 */
typedef struct 
{
    UINT8   aucUserMac[6];  /* MAC地址 */
    UINT32  ulAuthType;     /* 认证方式,用户密码为1,优惠券为2,固定密码为3,手 机为4,腾讯QQ为5,新浪微博为6,无密码 为7,微信为8,支付宝为9 */
    UINT64  ulUpLoad;       /* 上行流量,Byte */
    UINT64  ulDwLoad;       /* 下行流量,Byte */
    UINT32  ulConnect;      /* 连接数 */
    UINT32  ulTotalUp;      /* 累计上行流量,MB */
    UINT32  ulTotalDw;      /* 累计下行流量,GB */
    UINT32  ulConnectTime;  /* 连接时长,second */
    CHAR    acUserId[64];   /* 用户ID */
    UINT8   aucUserIp[4];   /* 用户IP */
    CHAR    acUserSsid[32]; /* 用户连接ssid */
    CHAR    acUserLoginTime[32];
    CHAR    acUserLogoutTime[32];
    UINT32  ulOnlineTime;   /* 在线时长 */
    CHAR    acLogoutType[32];   /* 下线方式 */
}AP_STAT_ONLINEUSER_ENTRY_T;

/* AP列表 */
typedef struct 
{
    UINT8   aucApMac[6];        /* AP的MAC地址 */
    UINT32  ulApNo;             /* 记录ID */
    UINT8   aucApIp[4];         /* AP IP */
    UINT32  ulCurLink;          /* 当前带机量 */
    UINT32  ulTotalLink;        /* 最大带机量 */
    UINT32  ulRoad;             /* 信道 */
    UINT32  ulTotalUp;          /* 累计上行流量,MB */
    UINT32  ulTotalDw;          /* 累计下行流量,GB */
    CHAR    acApMemo[128];      /* 备注 */
    UINT32  ulApStatus;         /* 运行状态,1为运行正常,0为运行异常 */
    CHAR    acApCpuStatus[64];  /* cpu运行状态 */
    UINT32  ulApRuntimeLong;    /* 运行时长 */
    CHAR    acApFirstSsid[32];  /* SSID1 */
    CHAR    acApSecondSsid[32]; /* SSID2 */
}AP_STAT_APINFO_ENTRY_T;
...
typedef struct 
{
    UINT8   aucApMac[6];    /* AP的MAC地址 */
    UINT32  ulApNo;         /* 记录ID */
    CHAR    acCurVer[32];   /* 当前版本号 */
    CHAR    acNewVer[32];   /* 最新版本号 */
    UINT32  ulApStatus;     /* 运行状态,1为运行正常,0为运行异常 */
}AP_STAT_VERSION_ENTRY_T;

/* AP连接终端列表 */
typedef struct 
{
    UINT8   aucApMac[6];    /* AP的MAC地址 */
    UINT8   aucStaMac[6];   /* 终端MAC */
    UINT32  ulStaNo;        /* 记录ID */
    CHAR    acStaName[32];  /* 终端名称 */
    UINT8   aucStaIp[4];    /* 终端IP */
    UINT32  ulRssi;         /* 信号强度 */
    UINT32  ulConnectTime;  /* 连接时长 */
    UINT32  ulStaTotalUp;   /* 累计上行流量，MB */
    UINT32  ulStaTotalDw;   /* 累计下行流量，GB */
    CHAR    acClientSsid[32];/* 终端连接SSID */
    CHAR    acLoginTime[32];/* 连接时间，格式yyyy-MM-dd HH:mm:ss */
}AP_STAT_AP_STA_ENTRY_T;

/* 网关对AP的操作 */
typedef struct
{
    UINT32  ulAction;           /* reboot(1), upgrade(2), update(3) */
    CHAR    acApMac[512];
    CHAR    acFirstSsid[32];
    CHAR    acSecondSsid[32];
    UINT32  ulTotalLink;
    UINT32  ulRoad;
    CHAR    acApMemo[64];
}AP_OPER_GWAP_T;
...
{   
    UINT32 ulNum;
    AP_SSID_T stSsidList[16];
}AP_SSID_LIST_T;

typedef struct {
	int iSwitch;			//无线认证启用开关,1为开启,0为关闭
	int iWeb_auth;			// Web认证启用开关,1为开启,0为关闭
	int iOffline; 			 //自动下线,超过时间无流量则强制其下线,0 为无限制,单位秒
	int iReverify;			 //重新认证,上网超过指定时须重新认证,0 为无限制,单位分钟
}SNMPAGENT_AUTH_T;
...
typedef struct
{
    UINT32 iUser_num; 		 //当前SSID下接入的用户数
    CHAR acIndex[8]; 	  //SSID对应的物理index，2.4G以0:开头，5G以1:开头。
    CHAR acAuth[16]; 	   //SSID 认证方式，可选值：open, share, wpa2-psk,wpa-psk
    CHAR acSsid[32]; 	   //SSID名
}SNMP_AP_SSID_INFO_T;

typedef struct
{
    UINT8 acMac[6];	//AP MAC-贝联
    UINT8 acIp[16];		//AP ip -贝联
    CHAR acfap_sn[32];	   //在当前网点内的AP标识，可以是ID号
    CHAR acDeviceType[32];	 //设备类型
    CHAR acLocation[64]; 	 //设备位置
    CHAR acVendor[32];	   //厂商标识
    UINT32 iOnline;			// 1-在线，0-离线是否在AC上线
    UINT32 iCha2;			//当前使用的2.4g信道-贝联
    UINT32 iCha5; 			//当前使用的5g信道
    UINT32 iRoam; 			 //漫游标记暂定为0
    UINT32 cur_link;			//当前带机量-贝联
    UINT32 total_link ;	// 最大带机量-贝联
    CHAR acMemo[128];     //备注-贝联
    UINT32 iRunStatus;    //运行状态,1为运行正常,0为运行异常0-贝联
    CHAR acCpu_status[64]; //cpu运行状态  "3% user,10% sys,87% idle"-贝联
    UINT32 total_up;		//累计上行流量-贝联
    UINT32 total_down;	//累计下行流量-贝联
    UINT32 runtimelong;	//运行时长-贝联
    CHAR softversion[64];	//软件版本
    UINT32 hardversion;	//硬件版本号
    UINT32 ap_work_mode;	//AP模式，0-->fit ; 1--> fat
    UINT32 iSsidNum;			 //ssid数目
    SNMP_AP_SSID_INFO_T ssid[16];
}SNMP_AP_INFO_T;
...
typedef struct{
	...
	UINT8 aucStaMac[6];
    UINT8 aucIp[4];
    CHAR acSsid[32];          /*接入的ssid名称*/
    CHAR acVapName[32];       /*接入的vap接口名称*/
    INT32 rssi;
    UINT32 connect_time;
    UINT64 total_up;
    UINT64 total_down;
...
typedef struct
{
   unsigned char mac[6];         //AP MAC地址
   SNMP_AP_INFO_T stApDetailInfo;
   UINT32 ulStaNum;
   SNMP_STA_INFO_T astStaInfo[128];
...
typedef struct
{
    ...
    /* BEGIN: Added by zoucaihong, 2015/3/25 */
    /* 贝联需求 */
    AP_CONFIG_USER_T                            stUserConfig;
    AP_CONFIG_GWSSID_T                          stGwSsidConfig;
    AP_CONFIG_AUTH_T                            stAuthConfig;
    AP_CONFIG_BATCH_BWLIST_T                    stBatchBwlistConfig;
    AP_CONFIG_DOMAIN_FILTER_T                   stUrlBwConfig;
    /* 中传需求 */
    AP_CONFIG_USER_SPEED_T                      stUserSpeedConfig;
    AP_CONFIG_USER_SPEED_T                      stWhiteUserSpeedConfig;
    AP_CONFIG_USER_LOGOUT_T                     stUserLogoutConfig;
    AP_CONFIG_GET_SECRET_KEY_T                  stGetSecretKeyConfig;
    /* END: Added by zoucaihong, 2015/3/25 */

	AP_STAT_SYS_T                               stSysStat;
    ...
    /* BEGIN: Added by zoucaihong, 2015/3/25 */
    /* 贝联需求 */
    AP_STAT_ONLINEUSER_ENTRY_T                  *pstOnlineUserInfo;
    UINT32                                      ulUserNum;
    AP_STAT_APINFO_ENTRY_T                      *pstApInfo;
    UINT32                                      ulApNum;
    AP_STAT_VERSION_ENTRY_T                     *pstApVersionInfo;
    AP_STAT_AP_STA_ENTRY_T                      *pstGwApStaInfo;
    /* END: Added by zoucaihong, 2015/3/25 */
