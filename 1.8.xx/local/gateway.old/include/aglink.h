...
#define HOTSPOT_NAME_LEN	64
	...
	u8 wan_mac[MAC_LEN]; 					/*设备WAN口MAC地址*/
...
#define AGLINK_LOG(__level__, __format__, __arg__...) \
	do{ \
		if((LOG_DEBUG == __level__) && (gpstaglinkmgt->debug_level != DEBUG_LEVER_NONE))\
			syslog(LOG_INFO, "[%s][%d] \n"__format__, __FUNCTION__, __LINE__, ##__arg__);\
		else if(LOG_DEBUG == __level__)\
			break;\
		else\
			syslog(__level__, "[%s][%d] \n"__format__, __FUNCTION__, __LINE__, ##__arg__);\
	}while(0)
