...
int stadct_send_list_report(void *func, void *config, int *sockfd)
{
	...
	for(i=0; i<ttl_sta_num; ++i){
		sprintf(temp, MAC_FMT"|%d;", MAC_ARG(g_mac_buf + i*STADCT_LEN), *(char *)(g_mac_buf + i*STADCT_LEN + 6));
...
int sta_dct_enable(int enable)
{
	int iRet = -1;
	unsigned char msg;
	static int enable_conf = 0;
	STA_DCT_LOG(LOG_INFO, "Send sta dct %s to aglink", (enable ? "enable" : "disable"));

	if(enable && !enable_conf){
	    if(0 != system("pidof locator_run"))
	        system("locator_run &");
		enable_conf = 1;
	}
