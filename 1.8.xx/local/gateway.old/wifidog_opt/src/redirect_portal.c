...
int do_wifidog_clear()
{
	...
	sys_log_set(WIFIDOG_LOG_FILE, "<<<<<<<<<<<< wifidog do clearing");
	fw_destroy();

	//删除正在运行downloadcloudsrc.sh httpdownload
	system("killall downloadcloudsrc.sh");
	system("killall httpdownload");
...
static void redirect_portal_select_loop(httpd *webServer,request *r,ssize_t *bytes,size_t *total_bytes, int max)
{
    int result;
    int maxfd = 0;
    fd_set fds;
...
void thread_first_page_handle(void)
{
    ...
    httpdAddCContent(firstpagehttpdServer, "/redirect", "", 0, NULL, http_callback_404);
    httpdAddCContent(firstpagehttpdServer, "/", "redirect", 0, NULL, http_callback_404);
...
int wifidog_check_http_port()
{
	...
	if(sys_config_get(value, "http_port") >= 0){
		port = atoi(value);
		if(port != 80)
			return 0;
	}
...
void http_port80_handle(void)
{
	...
	httpdAddCContent(httpdServer, "/redirect", "", 0, NULL, http_wxl_redirect_callback);
	httpdAddCContent(httpdServer, "/", "redirect", 0, NULL, http_wxl_redirect_callback);


        /*收到 alipay的离线token*/
	httpdAddCContent(httpdServer, "/login", "", 0, NULL, http_alipay_redirect_callback);
	httpdAddCContent(httpdServer, "/", "login", 0, NULL, http_alipay_redirect_callback);

	/*直接访问则重定向到BOA的8090端口*/
	httpdAddCContent(httpdServer, "/", "", 1, NULL, http_wxl_redirect2boa_callback);
...
void *thread_redirect(void *args)
{
	...
	if(FIRST_PAGE_TASK_NUM > 1)
		wifidog_set_thread_name(WIFIDOG_THREAD_8061MULTI);
	else
