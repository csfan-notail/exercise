...
extern int bblink_mac_baw_init(void);
extern int bblink_url_baw_init(void);
...
struct __thread_name g_threadname[WIFIDOG_THREAD_MAX] = {
	...
	{ "WD_8061MULTI",     	        WIFIDOG_THREAD_8061MULTI },
	{ "WD_STAKEY",     	            WIFIDOG_THREAD_STAKEY },
};
...
int wifidog_get_thread_index(unsigned int pid)
{
	...
	return WIFIDOG_THREAD_8061MULTI;
}
/*end: add@2015.01.28*/

//int g_debug_level = DEFAULT_DEBUGLEVEL;
int g_debug_level = __LOG_DEBUG__;
int g_daemon = 1;
...
{
    log_info(__LOG_FAILED__, "into sigchld_handler s %d", s);

	do_wifidog_clear();
	
	sys_log_set(WIFIDOG_LOG_FILE, "<<<<<<<<<<<< wifidog exit at sigchld_handler() %d", s);
	exit(1);
}
...
{
    log_info(__LOG_FAILED__, "into termination_handler s %d", s);
	
	do_wifidog_clear();
	
	sys_log_set(WIFIDOG_LOG_FILE, "<<<<<<<<<<<< wifidog exit at termination_handler() %d", s);
	exit(1);
}
...
static request *get_invalid_sockfd(request *r, int max)
{
    int loop;
    for(loop = 0; loop < max; loop++)
    {
    ...
    for(loop = 0; loop < r_len; loop++)
    {
        ...
        if(FD_ISSET(r[loop].authSock, fds))
        {
            ...
            else if(client_bytes[loop] == 0)
            {
                ...
                r[loop].readBuf[client_totalbytes[loop]] = '\0';
                log_info(__LOG_DEBUG__, "AuthServer Request Data [%s] [%d]", r[loop].readBuf, loop);
                client_bytes[loop] = client_totalbytes[loop] = 0;
                close(r[loop].authSock);
                r[loop].authSock = SOCKET_INVALID;
...
extern int user_http_ios_access_init(void);
...
static void main_loop(void)
{
    ...
    {
        if(NULL == (config->gw_address = get_iface_ip(config->gw_interface)))
        {
    ...
    if(!config->gw_id)
    {
        if(NULL == (config->gw_id = get_iface_mac(config->gw_interface)))
        {
    ...
    {
        if(NULL == (config->wan_mac = get_iface_mac(config->wan_interface)))
        {
	...
	/*add@2014.8.15*/
	/**@ first_page 处理线程: 处理first page 的http报文*/
	...
	/**@ 终端认证密钥获取线程 */
    if(pthread_create(&sta_key_handle_pid, NULL, (void*)thread_sta_key_handle, NULL) != 0)
    {
        log_info(__LOG_ERROR__, "Failed to create a new thread (gctl) - exiting");
		sys_log_set(WIFIDOG_LOG_FILE, "<<<<<<<<<<<< wifidog exit at main_loop() thread_sta_key_handle");
        exit(1);
    }
    log_info(__LOG_DEBUG__, "Create a new thread (gctl) success. pid %u", (unsigned int)sta_key_handle_pid);
    pthread_detach(sta_key_handle_pid);
	...
	/* @获取平台终端列表 */
	sys_log_set(WIFIDOG_MAINLOOP_FILE, "Fetch sta list.");
...
int main(int argc, char **argv)
{
	...
	sys_log_set(WIFIDOG_LOG_FILE, ">>>>>>>>>>> parent wifidog main start.");
    ...
    if(config->daemon)
    {
        ...
        switch(safe_fork()) {
			case 0: /* child */
				sys_log_set(WIFIDOG_LOG_FILE, "<<<<<<<<<<<< child wifidog start");
				setsid();
				main_loop();
				break;

			default: /* parent */
				sys_log_set(WIFIDOG_LOG_FILE, "<<<<<<<<<<<< parent wifidog exit at main()");
				exit(0);
				break;
		}
    }
