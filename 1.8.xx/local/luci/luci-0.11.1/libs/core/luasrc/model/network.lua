...
function get_avail_ifname(device)
	-- find all wifi-iface names
	local wifs = { }
	_uci_real:foreach("wireless", "wifi-iface",
		function(s)
	...
	local name = nil
	local device_id = device:match("wifi(%d)")
	if device_id then
		for i = 0,15 do
			local ifname = "ath%s%s" %{ device_id, i }
			if not wifs[ifname] then
				name = ifname
				break;
			end
		end
...
function add_wifinet(self, net, options)
	...
	then
		local wnet = _uci_real:section("wireless", "wifi-iface", get_avail_ifname(options.device), options)
		return wifinet(wnet)
	end
end
