...
function authenticator.htmlauth(validator, accs, default)
	...
	if nil ~= user then
		...
		for i=1,usernum  do
			username = username..i
			local getname = luci.util.exec("uci get webuser.@"..username.."[0].username")
			getname = getname.gsub(getname,"\n","")
			
			if user == getname then
				local get_pass = luci.util.exec("uci get webuser.@"..username.."[0].password")
				get_pass = get_pass.gsub(get_pass,"\n","")
				local get_auth = luci.util.exec("uci get webuser.@"..username.."[0].authority")
				get_auth = get_auth.gsub(get_auth,"\n","")
				if pass == get_pass then
					luci.sys.exec("uci set webuser.@user[0].authority="..get_auth)
					luci.sys.exec("uci commit webuser")
					luci.http.header("Set-Cookie", name.."user=")
					luci.http.header("Set-Cookie", name.."auth=")
					luci.http.header("Set-Cookie", name.."user="..user)
					luci.http.header("Set-Cookie", name.."auth="..get_auth)
					return user
				end
			end
			username = "user"
