...
--- Retrieve users allowed to login
-- @class		function
-- @name		getloginusers
-- @return		Table containing user names allowed to login

function user.getloginusers()
	local root = user.getuser("root")
	local alluser = user.getuser(nil)
	local loginusers = {}

	-- no root? impossible, return root as default
	if not root or not root["shell"] then
		return {"root"}
	end

	for k, v in pairs(alluser)
	do
		if v["shell"] == root["shell"] then
			loginusers[(#loginusers + 1)] = v["name"]
		end
	end
	
	local uci = require "luci.model.uci".cursor()
	--local usernum = uci.get('webuser','user','number')
	local usernum =luci.util.exec("uci get webuser.@user[0].number")
	local username = "user"
	...
	for i=1,usernum  do
		...
		loginusers[(#loginusers + 1)] =  luci.util.exec("uci get webuser.@"..username.."[0].username").gsub(luci.util.exec("uci get webuser.@"..username.."[0].username"),"\n","")

		username = "user"
	end
	
	return (#loginusers > 0) and loginusers or {"root"}
end
