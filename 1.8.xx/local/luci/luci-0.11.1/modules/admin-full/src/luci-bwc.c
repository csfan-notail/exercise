...
static int iwinfo_update(
	...
	if (!iw_get_rate)
	{
		...
		else if ((probe = dlsym(iw, "qcawifi_probe")) != NULL && probe(ifname))
		{
			iw_get_rate  = dlsym(iw, "qcawifi_get_bitrate");
			iw_get_rssi  = dlsym(iw, "qcawifi_get_signal");
			iw_get_noise = dlsym(iw, "qcawifi_get_noise");
		}
		else
		{
			return 0;
...
static void iwinfo_close(void *iw)
{
	...
	if ((do_close = dlsym(iw, "qcawifi_close")) != NULL) do_close();
