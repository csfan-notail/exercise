...
m.uci:foreach("network", "switch",
	function(x)
		...
		s = m:section(NamedSection, x['.name'], "switch",
			...
			switch_title and translatef("VLANs on %q ", switch_name)
		...
		local portvalue = function(self, section)
			...
			for pt in (m:get(section, "ports") or ""):gmatch("%w+") do
				...
				if self.option == "0" and pc == self.option then return (#tu > 0) and tu or "u" end
				if tonumber(pc) ==  5-tonumber(self.option) then return (#tu > 0) and tu or "u" end
			end
			return ""
		end
		...
		local portvalidate = function(self, value, section)
			...
			if value == "u" then
				...
				elseif min_vid > 0 or tonumber(self.option) ~= cpu_port then -- enable multiple untagged cpu ports due to weird broadcom default setup
						if 0 == tonumber(self.option) then 
							return nil,
								translatef("Port %d is untagged in multiple VLANs!", 0 )
						else 
							return nil,
								translatef("Port %d is untagged in multiple VLANs!", 5-tonumber(self.option) )
						end
				end
			end
			return value
			...
			for _, o in ipairs(port_opts) do
				...
				if v == "t" then
					if o.option == "0" then
						...
						p[#p+1] = 5-tonumber(o.option) .. v
					end
				elseif v == "u" then
					if o.option == "0" then
						p[#p+1] = o.option
					else
						p[#p+1] = 5-tonumber(o.option) .. ""
					end
				end
			end
		...
		for pt = 0, num_ports - 1 do
			...
			if pt == cpu_port then
				...
				title = translatef("Port %d", num_ports-pt)
			end
