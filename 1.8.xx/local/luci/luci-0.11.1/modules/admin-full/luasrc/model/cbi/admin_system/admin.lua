...
pw1.validate = function(self, value, section)
	local complexity = 0

        if value and string.len(value) < 5 then
		return nil,
			translate("Password number should not be less than 5!")
        end
--[[
        if value and nil ~= string.find(value,"%d+") then
        	complexity = complexity + 1
        end

        if value and  nil ~= string.find(value,"%l+") then
        	complexity = complexity + 1
        end

        if value and nil ~= string.find(value,"%u+") then
        	complexity = complexity + 1
        end

        if value and  nil ~= string.find(value,"[^%d%l%u]") then
        	complexity = complexity + 1
        end

        if complexity < 3 then
		return nil,
                        translate("Given password must include big, lowercase letters, Numbers, special symbols such as class character of 3 class 4 or above!")
         end
]]--                
	return value
end

pw2.validate = function(self, value, section)
	local v1 = pw1:formvalue("_pass")	
        
	if v1 and value and v1 == value then
                return value
        end
        
        return nil,
                translate("Given password confirmation did not match, password not changed!")
end
	...
	if v1 and v2 and #v1 > 0 and #v2 > 0 then
		if v1 == v2 then
			if luci.sys.user.setpasswd(luci.dispatcher.context.authuser, v1) == 0 then
				os.execute("uci set webuser.@user1[0].password="..v1)
				os.execute("uci commit webuser")	
