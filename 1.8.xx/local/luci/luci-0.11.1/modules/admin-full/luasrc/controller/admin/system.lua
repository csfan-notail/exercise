...
function index()
  ...
  if not require("luci.custom").strip_en() then
	...
	entry({"admin", "system", "flashops", "vflagstatus"}, call("action_vflag_status")).leaf = true

	entry({"admin", "system", "reboot"}, call("action_reboot"), _("Reboot"), 90)
end

-- Version upgrade 
version_tmp     = "/tmp/app.bin"
vflag_log       = "/tmp/vflag_local_update.log"	
vflag_err       = "/tmp/vflag_local_update_err.log"
vflag_unavail   = os.execute("type vflag >/dev/null 2>&1") ~= 0
vflag_cmd       = string.format("rm %s; vflag localupdate %s > %s 2>&1 && rm %s || mv %s %s &", 
								vflag_err, version_tmp, vflag_log, vflag_log, vflag_log, vflag_err)

local function version_valid()
	return luci.fs.access(version_tmp) and os.execute("vflag check "..version_tmp.." >/dev/null") == 0
end
	
function action_vflag_status()
	local data = nixio.fs.readfile(vflag_log)
	...
	if data then
		...
		luci.http.write(lastline)
	else
		local err_msg=nixio.fs.readfile(vflag_err)
		luci.http.write("#luci#version#update#finish#")
		if err_msg then
...
function action_flashops()
	...
	luci.http.setfilehandler(
		function(meta, chunk, eof)
			if not fp then
				...
				elseif meta and meta.name == "version" then
					fp = io.open(version_tmp, "w")
				else
	...
	elseif luci.http.formvalue("version") or luci.http.formvalue("version_step") then
		-- version_step: 1 ~ before upgrade, 2 ~ in progress
		local version_step = tonumber(luci.http.formvalue("version_step") or 1)
		if version_step == 1 then
			local version_inval = not version_valid()
			if vflag_unavail or version_inval then
				nixio.fs.unlink(version_tmp)
				luci.template.render("admin_system/flashops", {
					reset_avail   = reset_avail,
					upgrade_avail = upgrade_avail,
					vflag_unavail = vflag_unavail,
					version_inval = version_inval
				})
			else
				luci.template.render("admin_system/version_upgrade", {
	...
	elseif luci.http.formvalue("image") or luci.http.formvalue("step") then
		...
		elseif step == 2 then
			...
			luci.template.render("admin_system/applyreboot", {
				...
				addr  = (#keep > 0) and "172.16.0.1" or nil
			})
	...
	elseif reset_avail and luci.http.formvalue("reset") then
		...
		fork_exec("killall dropbear uhttpd; sleep 1; restore -a")
	else
		--
		-- Overview
		--
