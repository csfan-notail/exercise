...
int pal_wifi_get_wifi_info(WIFI_INFO_T *pstWifiInfo,unsigned int wifi_id)
{
	char uci_name[128];
	char uci_value[128];
...
int pal_wifi_ssid_encode(unsigned char *dev,unsigned char *ssid)
{
	char nvram_name[128];
	char nvram_value[128];
	...
	gw_log(LOG_DEBUG, "dev %s ssid %s", dev,ssid);

	if(!strcmp(dev,"ath0"))
		memcpy(ath_name,"ath00",sizeof("ath00"));
	else if(!strcmp(dev,"ath1"))
		memcpy(ath_name,"ath10",sizeof("ath10"));
	else
		memcpy(ath_name,dev,strlen(dev));

	sprintf(nvram_name,"wireless.%s.encode",ath_name);
	memset(ssid_encode,0,sizeof(ssid_encode));
	if(!nvram_get(ssid_encode,nvram_name,128) && !strncmp(ssid_encode,"GB2312",strlen("GB2312"))){
			sprintf(acCmd, "echo %s > /tmp/ssid_encode_UTF-8", ssid);  
			system(acCmd);
			sprintf(acCmd,"iconv -c -s -f GB2312 -t UTF-8 /tmp/ssid_encode_UTF-8");
			temstrp = cmd_return_str(acCmd);		
			memset(ssid,0,strlen(ssid));
			memcpy(ssid,temstrp,strlen(temstrp));
