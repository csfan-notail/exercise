...
int portal_get_auth_state(unsigned char *pucMac,unsigned int *puiState)
{
...
int portal_set_auth_state(unsigned char *pucMac, unsigned int uiState)
{
    AP_ERROR_CODE_E enRet;
    NF_PORTAL_IOCTL_AUTH_STATE_T stIoAuthState;
...
/*目的网段黑名单，添加，删除，清空*/
int portal_add_dest_net_black_list(unsigned int uiNet,unsigned int uiMaskNum)
{
    NF_PORTAL_IOCTL_NET_BAW_T stIoNetBaw;
    AP_ERROR_CODE_E enRet;

    stIoNetBaw.stNetBaw.uiNet = ntohl(uiNet);/*字节序转换*/
    stIoNetBaw.stNetBaw.uiMaskNum = uiMaskNum;
    stIoNetBaw.stNetBaw.enFlag = BLACK;
...
int portal_del_dest_net_black_list(unsigned int uiNet,unsigned int uiMaskNum)
{
    NF_PORTAL_IOCTL_NET_BAW_T stIoNetBaw;
    AP_ERROR_CODE_E enRet;
...
int portal_add_https_url_free_auth_list(unsigned char *pucUrl)
{
    ...
    memcpy(stIoUrlBaw.stUrlBaw.aucUrl,pucUrl,URL_MAX_SIZE);
    stIoUrlBaw.stUrlBaw.enFlag = HTTPS_FREE_AUTH;
...
/*URL免广告名单，添加，删除，清空*/
int portal_add_free_advertising_url_list(unsigned char *pucUrl)
{
    NF_PORTAL_IOCTL_URL_BAW_T stIoUrlBaw;
    AP_ERROR_CODE_E enRet;
    ...
    memcpy(stIoUrlBaw.stUrlBaw.aucUrl,pucUrl,URL_MAX_SIZE);
    stIoUrlBaw.stUrlBaw.enFlag = FREE_ADVERTISING;
...
int portal_del_free_advertising_url_list(unsigned char *pucUrl)
{
    NF_PORTAL_IOCTL_URL_BAW_T stIoUrlBaw;
    AP_ERROR_CODE_E enRet;
...
int portal_flush_free_advertising_url_list(void)
{
    NF_PORTAL_IOCTL_URL_BAW_T stIoUrlBaw;
    AP_ERROR_CODE_E enRet;
...
int portal_add_wxl_serverip(unsigned int uiIp, unsigned int uiMask)
{
    ...
    stIoWxlServerIp.stWxlServerIp.uiIp = ntohl(uiIp);
