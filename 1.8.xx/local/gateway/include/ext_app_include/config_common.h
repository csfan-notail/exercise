...
typedef enum _ccs_module_config_check_
{
    CCS_MODULE_CONFIG_CHECK_ERROR,
    CCS_MODULE_CONFIG_CHECK_OK,
}CCS_MODULE_CONFIG_CHECK_E;
...
typedef struct _sntp
{
    ...
    CCS_MODULE_CONFIG_CHECK_E eCheckStatus;
}sntp;
...
typedef struct _dhcp_server
{
    ...
    int reboot_id;                     /*修改了默认地址池LAN IP是否重启 */
    int num;                        /* DHCP地址池个数 */
...
typedef struct
{
	char aAuditServerSwitch[8]; //switch
	char acAuditServerAddr[128]; //server Addr
	char acAuditNetworkName[128]; //上网场所名称
	char acAuditNetworkAddr[256]; //上网场所地址
	char acAuditDevAddr[64]; //设备名称
	char acAuditLongitude[64]; //AP经度
	char acAuditLatitude[64]; //AP维度
	char acDevMac[64]; //设备mac地址
	//add by qiu 2015/12/21
	char mirrorUpSwitch[8];    //mirror switch
	char mirrorUpProtocol[8];  //send mirror protocol udp: 0, tcp: 1
	char dataUpSwitch[8];      //user log switch
	char staCollectionSwitch[8];//MAC collection switch
	char featureSwitch[8];        // feature switch      
	char analogMacSwitch[8];   //analog sta switch
	//char authflg[8]; //authentication flag
}AUDIT_SERVER_CONFIG_T;



typedef struct
{
    char aucVpnPptpSwitch[32];
    char aucVpnIpsecSwitch[32];
}VPN_PASS_CONTROL_INFO_T;

#ifdef AD_SUPPORT
typedef struct
{
    char ad_switch[32]; /*ad switch*/
    char acAdJsServer[64]; /*Ad Js server*/
    char acAdServer[64]; /*Ad server*/
}AD_CONFIG_T;
#endif
...
typedef struct
{
    char pcap_switch[32]; /*pcap switch*/
    char pcap_dev[32];     /*pcap dev*/
    char pcap_proto[32];  /*pcap proto*/
    char pcap_port[32];    /*pcap port*/
...
{
    int ios_auto_portal;
	int ios_redirect_mode;
	int uc_redirect_mode;
}authorize_control_t;
...
typedef struct
{
	...
	char channelSwitch[CONFIG_VALUE_LEN];
...
/* timed_task.html add 2015-9-14*/
typedef struct _timed_task
{
	int taskswitch;
	int cycle;
	int mday; //日期
...
#define OM_FREEAUTH_MAC_MAX 1024
#define OM_FREEAUTH_MAC_PERLINE 16

typedef struct 
{
	unsigned char aucMac[6];
	int ulFreeAuthMacIndex;
}OM_FREEAUTH_MAC_CFG_T;


typedef struct _other_access_control_config
{
    int  lan_typemodel;
	int  vlan_flood;   /*VLAN间广播*/
	int  lan_isolation;	/*LAN口隔离:  0:禁用  1:启用*/
	int  reserve_port_map; /*保留端口映射:  0:禁用  1:启用*/
}other_access_control_config;


typedef struct
{
    int uiSpanSsidAuthCtrl;
}SPAN_SSID_AUTH_CTRL_T;
