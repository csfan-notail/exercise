...
extern int set_backup(int flag, char* errmsg);
...
extern int recover_config(char *name, char* errmsg);
extern int copy_config(char *name, char* errmsg);
...
extern int set_wifidog_conf(char *hotspot_name, char *hotspot_url, char *hotspot_path,
	char *auth_ctrl, char *ccs_server, char *snmp_server, char *server_mode,
	char *wxl_ssid, char *wxl_bssid, char *http_ad_switch, char *http_ad_wl_dura,
	char *http_ad_bl_dura, char *http_rd_maxcnt, char *http_ck_Intvl, char *http_ck_Stalimit);
extern int set_free_auth(char *port_name, int auth_state);
/*begin:@2015-8-19 for lan_free_auth*/
extern int set_lan_free_auth(int auth_state);
/*end:*/
extern int set_network_model(int net_model);
...
extern int network_ipv4_config2(ipv4 *pst_ipv4, ipv4 *pst_ipv4_old, int flag, int id);
extern int network_forward_config(void);
extern int network_check_http_access(char *szHttpReq, char *szHttpRsp, int iTimeOut);
...
extern void del_str_for_douhao(char *str, char *str_n);
...
extern int determine_to_join(dhcp_server *st_dhcp, int iLoop, int *vlan_list, char *vlan);
extern void remove_unexit_vlan(char *vlan_band, char *vlan_all);
extern int fireware_set(int mode, int flag, int id);
...
extern int check_lan_ip_conflict(void);
/*----------------------- wireless_interface.c -----------------------*/
extern int vap_info_config(vap *pst_vap, int flag);
int vap_get_vapnum_by_bssid(char *pBssid);
...
extern int  om_freeauth_mac_get(OM_FREEAUTH_MAC_CFG_T *pstFreeAuthMacCfg);
extern int  om_freeauth_mac_del(OM_FREEAUTH_MAC_CFG_T *pstFreeAuthMacCfg);
extern int  om_freeauth_mac_add(OM_FREEAUTH_MAC_CFG_T *pstFreeAuthMacCfg);
extern int  om_freeauth_get_mac_list(OM_FREEAUTH_MAC_CFG_T *pstFreeAuthMacCfg, int *piNum);
extern int  om_freeauth_mac_init(void);



#ifdef AD_SUPPORT
extern int advertisement_set(AD_CONFIG_T *pstAd);
#endif

#ifdef PCAP_SUPPORT
extern int pcap_set(PCAP_CONFIG_T *pstPcap);
#endif
