...
enum AGLINK_MSG_ID
{
	...
	AGLINK_CLI_SET_DEBUG_LEVEL,
	AGLINK_CLI_SEND_APINFO_TO_GW,
	AGLINK_CLI_MAX,
	AGLINK_MSG_MAX
};
...
enum AGLINKCLI_MSG_ID
{
	...
	AGLINK_CLI_SEND_APINFO_TO_GW_ACK,
	AGLINKCLI_MSG_MAX
};
enum BOXLL_MSG_ID
{
...
enum locator_swmsg{
	LOCATOR_DISABLE_WIFIDOG = 0,
	LOCATOR_ENABLE_WIFIDOG,
	LOCATOR_DISABLE_AUDIT,
	LOCATOR_ENABLE_AUDIT
};
...
enum locator_idset{
	...
	LOCATOR_IDSET_SW,
	LOCATOR_IDSET_RSSI,
	LOCATOR_IDSET_QUITTIMEOUT,
	LOCATOR_IDSET_MAX
};

enum AUDIT_MSG_ID
{
    AUDIT_MSG_BEGIN = 0x0a000000,
    AUDIT_MSG_STA_ON_OFF,
	AUDIT_MSG_LOCATOR_ADD_STA,
	AUDIT_MSG_LOCATOR_DEL_STA,
    AUDIT_MSG_MAX
};
...
typedef struct ag_module_register
{
	...
	struct event recv_ev;					/* 接收消息事件 */
       u16 uiReconnectTvFlag;  /*connect_ipc_server_tv 定时任务是否执行1:未执行  0:没有任务待执行*/
...
#ifdef OPENWRT
#define SSID_NUMBER			32
#else
#define SSID_NUMBER			16
#endif


 typedef struct {
	 s32 iUser_num; 		 //当前SSID下接入的用户数
	 u8 acIndex[8]; 	  //SSID对应的物理index，2.4G以0:开头，5G以1:开头。 apX_X
	 u8 acAuth[16]; 	   //SSID 认证方式，可选值：open, share, wpa2-psk,wpa-psk
	 u8 acSsid[32]; 	   //SSID名
 } AGLINK_AP_SSID_INFO_T;


 typedef struct{
	 s32 iSsidNum;
	 AGLINK_AP_SSID_INFO_T ssid[SSID_NUMBER];
 }AGLINK_AP_SSID_CFG_T;

 typedef struct {
	  u8 acMac[MAC_LEN];		 //AP MAC地址-贝联
	  u8 acLocation[NVRAM_LEN*2];	 //设备位置	"一楼"	瘦：config区，胖：nvram区
	  s32 iCha2;		 	//当前使用的2.4g信道-贝联
	  s32 iCha5;		 	//当前使用的5g信道
	  s32 total_link ;		// 最大带机量-贝联
	  u8 acMemo[NVRAM_LEN*4];	//备注-贝联
	  s32 iSsidNum; 		//ssid数目
	  AGLINK_AP_SSID_INFO_T ssid[SSID_NUMBER];
} AGLINK_AP_CONFIG_INFO_T;


typedef struct {
   u8 acMac[MAC_LEN];  //AP MAC-贝联
   u8 acIp[IP_LEN];        //AP ip -贝联
   u8 acfap_sn[32];       //在当前网点内的AP标识，可以是ID号
   u8 acDeviceType[32];     //设备类型
   u8 acLocation[64];   //设备位置
   u8 acVendor[32];       //厂商标识
   s32 iOnline;            // 1-在线，0-离线是否在AC上线
   //u8 iChannel[32];          //当前使用的信道
   s32 iCha2;          //当前使用的2.4g信道-贝联
   s32 iCha5;          //当前使用的5g信道
   s32 iRoam;           //漫游标记暂定为0
   s32 cur_link;           //当前带机量-贝联
   s32 total_link ;    // 最大带机量-贝联
   u8 acMemo[128];     //备注-贝联
   s32 iRunStatus;    //运行状态,1为运行正常,0为运行异常0-贝联
   u8 acCpu_status[64]; //cpu运行状态  "3% user,10% sys,87% idle"-贝联
   u32 total_up;       //累计上行流量-贝联
   u32 total_down; //累计下行流量-贝联
   u32 runtimelong;    //运行时长-贝联
   u8 softversion[64]; //软件版本
   u32 hardversion;    //硬件版本号
   u32 ap_work_mode;   //AP模式，0-->fit ; 1--> fat
   s32 iSsidNum;            //ssid数目
   AGLINK_AP_SSID_INFO_T ssid[SSID_NUMBER];
} AGLINK_AP_DETAIL_INFO_T;

typedef struct{
   u8 aucUrl [128];        //版本地址
   s32 iType;              //版本下载方式  
   u8 aucServerIP[4];      //server IP
   u32 aucServerPort;  //server 端口
   u8 aucUser[32];     //ftp 用户名
   u8 aucPasswd [16];    // ftp 密码
}VERSION_UPDATE_T;
...
typedef struct{
	...
	u32 connect_time;
    	u64 total_up;
    	u64 total_down;
}WIFIOG_EVENT_ADD_STA_T,STAM_EVENT_ADD_STA_T, BOXLL_EVENT_STA, SNMP_EVENT_STA_INFO;
...
typedef struct{
    ...
    char cRssi;
	char acSsid[64];
	char acApChannel[4];  /*扫描到终端的信道，如1、6、11，字符串类型*/
	char acEncryptType[4]; /*加密类型，如01(WEP) 02(WPA)，字符串类型*/
}AUDIT_LOCATOR_ADD_STA_T;
/*WIFIDOG_IDSET_LOCATOR_WIFIDOG_DEL_STA 的消息体*/
typedef struct{
	unsigned char aucApMac[MAC_LEN];
	unsigned char aucStaMac[MAC_LEN];
}AUDIT_LOCATOR_DEL_STA_T;
