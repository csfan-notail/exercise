...
FC_PKT_STATE_E (*fc_enqueue_hook)(struct sk_buff *pstSkb, FC_DIR_E enDir);
EXPORT_SYMBOL(fc_enqueue_hook);
...
rx_handler_result_t br_handle_frame(struct sk_buff **pskb)
{
    ...
    if (NULL != fc_enqueue_hook)	
    {
       FC_PKT_STATE_E enPktState;
    	enPktState = fc_enqueue_hook(skb, FC_DIR_US);
    	if (FC_PASS_PKT != enPktState)
    	{
    		return RX_HANDLER_CONSUMED; /*报文处理完毕*/
    	}
    }
forward:
	switch (p->state) {
