 ...
 *----------------------------------------------*/
 ...
 extern int aglink_log_set(char *name, const char *format, ...);
...
#define AG_MODULE_ORDER 	11
u8 *gucModuleorder[AG_MODULE_ORDER] = {"aglink", "aglink", "aglink", "stam", "snmpagent", "wifidog","Aglinkcli","Boxll","Stadct"," ","audit"};
...
u32 ag_module_ipc_server_send_msg_to_internal(AG_LINK_MSG_HEAD_T *pstMsgHead, u8 *pstBuffer, int iTotallen, struct sockaddr_in *sk_addr)
{
	...
	SEND_UNREGISTER:
	{
		...
		struct sockaddr_in  st_addr = {0};
              ...
              if(AG_GET_MODULE_ID(uiMsgid) >= AG_MODULE_ORDER)
              {
                    AG_MODULE_LOG(LOG_ALERT,"*******uiMesgId=%d over %d unkown module\n",AG_GET_MODULE_ID(uiMsgid),AG_MODULE_ORDER);
                    return FALSE;
              }
		
		
		AG_MODULE_LOG(LOG_ALERT, "Can`t  find module+1:%s(%d)", gucModuleorder[AG_GET_MODULE_ID(uiMsgid)], AG_GET_MODULE_ID(uiMsgid));
...
s32 main(s32 argc, u8 **argv)
{
	...
	AG_MODULE_LOG(LOG_INFO, "aglink start to init.");
        ...
        aglink_log_set(AGLINK_AGENTD_LOG ,">>>>>>>aglink start run>>>>>>>");
