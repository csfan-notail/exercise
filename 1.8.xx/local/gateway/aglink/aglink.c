...
#endif
#endif
#define AGLINK_AGENTD_LOG "/tmp/aglink_agentd.log"
...
int gAglinkDebugLevel = LOG_INFO;
...
#include <time.h>
int aglink_log_set(char *name, const char *format, ...)
{
...
static s32 aglink_get_dev_model_id()
{
		...
		return IModelid;
            ...
            if(!factory_get(dev_model_id,DEVICE_MODEL_ID,32))
                return 7005001;
            else 
...
s32 aglink_get_wan_ip(u8 *wan_ip)
{
    ...
    if(IP_MODEL_3G_4G_LTE == atoi(nvram_value)){
        strcpy(wanport, "usb0");
    }
    else if(IP_MODEL_PPPOE_CLIENT == atoi(nvram_value)){
        strcpy(wanport, "ppp0");
    }
...
/*begin:aglinkcli 相关处理函数2015-9-10*/
s32 aglink_set_debug_level(u8 **pnt, int iLength)
{
	char cLevel;
	int uiLevel=0;
	cLevel = (**pnt);
	AGLINK_LOG(LOG_INFO, "old debug level :%d     newlevel length:%d  level=%c\n", gAglinkDebugLevel,iLength,cLevel);
	if(iLength > 0)
	{
	    uiLevel = cLevel - '0';
	    if(uiLevel >= 0 && uiLevel <=7)
	    {
               gAglinkDebugLevel = uiLevel;
            }
            AGLINK_LOG(LOG_INFO, "newLevel = %d gAglinkDebugLevel :%d\n", uiLevel,gAglinkDebugLevel);
	}
	AGLINK_LOG(LOG_INFO, "set over\n");
	return 0;
}

/*end:aglinkcli 相关处理函数2015-9-10*/
...
{	
	...
	if(stSsidCfg.iSsidNum != gpstaglinkmgt->pAgconfig->iSsidNum ||memcmp(stSsidCfg.ssid, gpstaglinkmgt->pAgconfig->ssid, sizeof(AGLINK_AP_SSID_INFO_T)*SSID_NUMBER))
	{
		...
		if(1 == gpstaglinkmgt->pstdev_info->stAgwifiInfo[1].iflag)
		{
			if(FALSE == aglink_agconfig_ssid(stSsidCfg.ssid, iIndex, 1, iMax))
			{
				return FALSE;
			}
		}
...
s32 aglink_parse_ag_msg(u8 *buf, u32 len, struct sockaddr_in *sk_addr)
{
	...
	AGLINK_LOG(LOG_INFO, "msg from Module name:%s MsgId:%x MsgType:%d, MsgLen:%d", stMsgHead.acname, stMsgHead.usMsgId, stMsgHead.usMsgType, stMsgHead.uiMsgLen);
	...
	case AGLINK_MSG_DEV_REGISTER_MSG:
              AGLINK_LOG(LOG_DEBUG, "recv AGLINK_MSG_DEV_REGISTER_MSG");
		ret = aglink_gw_recv_dev_register_msg(pnt, sk_addr);
		break;
	case AGLINK_MSG_DEV_REGISTER_ACK:
              AGLINK_LOG(LOG_DEBUG, "recv AGLINK_MSG_DEV_REGISTER_ACK");
	...
	case AGLINK_MSG_AP_INFO:
		AGLINK_LOG(LOG_DEBUG, "recv AGLINK_MSG_AP_INFO");
		aglink_gw_recv_ap_reportinfo_msg(pnt, sk_addr);
		break;
	#if 0	
	case AGLINK_IDSET_DEV_LOGOUT_ACK:
		AGLINK_LOG(LOG_DEBUG, "recv dev logout ack msg.");
		break;
	case AGLINK_IDSET_STA_ADD:
		AGLINK_LOG(LOG_DEBUG, "recv AGLINK_IDSET_STA_ADD msg");
		aglink_gw_recv_sta_connect_msg(pnt, stMsgHead.usMsgId);
		break;
	case AGLINK_IDSET_STA_DEL:
		AGLINK_LOG(LOG_DEBUG, "recv AGLINK_IDSET_STA_DEL msg");
		aglink_gw_recv_sta_connect_msg(pnt, stMsgHead.usMsgId);
		break;
	#endif		
	#ifndef GW_X86
	...
	case STA_DETECT_RSSI_MSG:
		AGLINK_LOG(LOG_DEBUG, "recv sta detect rssi msg");
...
s32 aglink_parse_agcli_msg(u8 *buf, u32 len, struct sockaddr_in *sk_addr)
{
	...
	switch(stMsgHead.usMsgId)
	{
		...
		case AGLINK_CLI_SET_KEEPLIVE_TM:
			...
			break;
#endif	
            case AGLINK_CLI_SET_DEBUG_LEVEL:
	       {
                    AGLINK_LOG(LOG_INFO, "recv AGLINK_CLI_SET_DEBUG_LEVEL");
                    aglink_set_debug_level(pnt, stMsgHead.uiMsgLen);
                    break;
	       }
		default:
...
u32 aglink_send_ag_msg(u32 usMsgID, u16 uiMsgType, u8 *pbuff, u32 iMsgLen, u8 *dmac)
{
	...
	else
	{
		...
		AGLINK_LOG(LOG_DEBUG, "MsgId:%x MsgType:%d ModuleName:%s MsgLen:%d uitotallen:%d AG_LINK_MSG_HEAD_T:%d ", 
...
s32 aglink_send_apmsg_to_wifidog(u8 * mac, u8 * ip, u32 model_id, s8 * dev_name, AP_WIFI_INFO *pstApInfo, s32 type)
{
	...
	if(NULL != pstApInfo)
	{
	       pstApInfo->uiRadioNum = htonl(pstApInfo->uiRadioNum);
	       for(iLoop=0;iLoop<AG_MODULE_AP_MAX_RADIO_NUM;iLoop++)
	       {
	            pstApInfo->astRadio[iLoop].uiRadioType = htonl(pstApInfo->astRadio[iLoop].uiRadioType);
	            pstApInfo->astRadio[iLoop].uiVapNum = htonl(pstApInfo->astRadio[iLoop].uiVapNum);
	       }
	...
	/*begin:2015-9-10 跟新ap信息*/
	else if(DEV_REPORT_MSG == type)
	{
            ag_module_ipc_server_send_msg(WIFIDOG_MSG_AP_REPORT_APINFO, AG_IPC_MSG_TYPE_SELF_CPU, "aglink", (u8 *)&stMsg, sizeof(WIFIOG_AP_REGISTER_T), NULL);		
	}
	/**/
	else
	{
...
s32 aglink_gw_recv_dev_register_msg(u8 **pnt, struct sockaddr_in *sk_addr)
{
	if(IS_DEV_GW())
	{
		...
		for(iLoop = 0; iLoop < AG_MODULE_AP_MAX_RADIO_NUM; iLoop++)
		{
			AGLINK_LOG(LOG_DEBUG, "RadioId:%d , vapNum:%d",stApInfo.astRadio[iLoop].uiRadioType, stApInfo.astRadio[iLoop].uiVapNum);
			for(iInLoop = 0; iInLoop < stApInfo.astRadio[iLoop].uiVapNum; iInLoop++)
			{
				AGLINK_LOG(LOG_DEBUG, "VapName:%s SsidName:%s Bssid:"MAC_FMT, 
		...
		AGLINK_LOG(LOG_DEBUG, "register dev: "MAC_FMT" "IP_FMT" model_id %d name %s",
...
s32 aglink_gw_recv_dev_keepalive_msg(u8 **pnt, struct sockaddr_in *sk_addr)
{
	...
	if(IS_DEV_GW())
	{
		...
		AGLINK_LOG(LOG_DEBUG, "total:%d used:%d", stDiskInfo.total, stDiskInfo.used);
		...
		AGLINK_LOG(LOG_DEBUG, "CPU: %d (system=%d user=%d nice=%d idle=%d)", 
		...
		AGLINK_LOG(LOG_DEBUG, "WAN stat: RecvBytes:%lu SendBytes:%lu", stWANstat.RecBytes, stWANstat.SendBytes);
...
s32 aglink_gw_send_dev_keepalive_ack_msg(u8 *mac)
{
	...
	uiLength = p - &buf[0];
	AGLINK_LOG(LOG_DEBUG, "send dev keepalive ack msg. dmac "MAC_FMT, MAC_ARG(mac));
...
/*begin:2015-9-10 ap report info*/
s32 aglink_gw_recv_ap_reportinfo_msg(u8 **pnt, struct sockaddr_in *sk_addr)
{
    if(IS_DEV_GW())
    {
            struct hlist_node *pos;
            ...
            for(iLoop = 0; iLoop < AG_MODULE_AP_MAX_RADIO_NUM; iLoop++)
            {
                AGLINK_LOG(LOG_DEBUG, "RatioId:%d , vapNum:%d",stApInfo.astRadio[iLoop].uiRadioType, stApInfo.astRadio[iLoop].uiVapNum);
                for(iInLoop = 0; iInLoop < stApInfo.astRadio[iLoop].uiVapNum; iInLoop++)
                {
                    AGLINK_LOG(LOG_DEBUG, "VapName:%s SsidName:%s Bssid:"MAC_FMT, 
                        stApInfo.astRadio[iLoop].astVap[iInLoop].acVapName, stApInfo.astRadio[iLoop].astVap[iInLoop].acSsid, MAC_ARG(stApInfo.astRadio[iLoop].astVap[iInLoop].aucBssid));
            ...
            AGLINK_LOG(LOG_DEBUG, "recv report ap info: "MAC_FMT" "IP_FMT" model_id %d name %s",
                MAC_ARG(new_ap->mac), IP_ARG(new_ap->ip), new_ap->model_id, new_ap->dev_name);
            ...
            {/*跟新信息*/
                AGLINK_LOG(LOG_DEBUG, "update AP "MAC_FMT, MAC_ARG(new_ap->mac));
                memcpy(&ap->time, &new_ap->time, sizeof(struct timeval));
                memcpy(&ap->sk_addr, &new_ap->sk_addr, sizeof(struct sockaddr_in));
                ...
                if(strncmp(ap->dev_name, new_ap->dev_name, 64)){
                    ...
                    aglink_send_apmsg_to_wifidog(ap->mac, ap->ip, 
                        ap->model_id, ap->dev_name, &stApInfo, DEV_REPORT_MSG);
                MEM_FREE(new_ap);
            }
            AP_HLIST_UNLOCK();   
        }
...
s32 ag_module_init_aglink(u8 *pagmodule)
{
	...
	if(NULL == (gpstaglinkmgt->pAgconfig = MEM_CALLOC(sizeof(AGLINK_AP_CONFIG_INFO_T))))
	{
        ...
        gpstaglinkmgt->pstdev_info->ap_work_mode = aglink_get_ap_work_mode();
        if(AGLINK_FIT_AP == gpstaglinkmgt->pstdev_info->ap_work_mode)
        {/*瘦ap*/
            AGLINK_LOG(LOG_INFO, "get aglink dev work_mode: FIT_AP_MODE.");
            aglink_log_set(AGLINK_AGENTD_LOG ,"dev work_mode: FIT_AP");
            gpstaglinkmgt->pstdev_info->dev_type = DEV_TYPE_AP;
        }
        else
        {/*胖ap 或者网关*/
            if(AGLINK_CONFIG_GET(forward_mode, NET_FORWARD_MODE, (NVRAM_LEN*2)) != 0)
            {
                aglink_log_set(AGLINK_AGENTD_LOG ,"nvram get net_forward_mode error");
                return -1;
            }
            else if(!strcmp(forward_mode, "2"))
            {/*ap mode*/
                aglink_log_set(AGLINK_AGENTD_LOG ,"dev work_mode: FAT_AP");
                gpstaglinkmgt->pstdev_info->dev_type = DEV_TYPE_AP;
            }
            else
            {
                gpstaglinkmgt->pstdev_info->dev_type = DEV_TYPE_GW;
                aglink_log_set(AGLINK_AGENTD_LOG ,"dev work_mode: GATEWAY");
