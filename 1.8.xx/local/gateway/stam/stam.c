...
s32 stam_module_get_essid(u8 *dev, u8 *ucEssid)
{
	...
	#ifdef OPENWRT
	/*gb2312 need  encode to utf-8*/
	pal_wifi_ssid_encode(dev,ucEssid);
	#endif
	return TRUE;
}
...
{
	s32 iRet = 0;
	char ip_method[NVRAM_LEN]={0};

	#ifdef LTE_SUPPORT
	/*判断WAN口接入模式*/
	iRet = nvram_get(ip_method, NVRAM_IPv4"_"NVRAM_IP_METHOD, (NVRAM_LEN*2));
	if(atoi(ip_method)== 3)
		stam_module_get_if_mac(gcWanMac,   "usb0");
	else
		stam_module_get_if_mac(gcWanMac,   "eth0");
	#else
