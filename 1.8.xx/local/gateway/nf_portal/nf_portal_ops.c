...
AP_ERROR_CODE_E nf_portal_ioctl_set_auth_state(NF_PORTAL_IOCTL_AUTH_STATE_T *pstIoAuthState)
{
    ...
    pstEntry = nf_portal_get_sta_from_htable(pstIoAuthState->aucMac);
	...
	else if(STATE_DEL == pstIoAuthState->enState)
    {
...
{
    NF_PORTAL_STA_ENTRY_T *pstEntry = NULL;
    if (unlikely(NULL == pstIoStaType))
    {
        return AP_E_PARAM;
    }

    pstEntry = nf_portal_get_sta_from_htable(pstIoStaType->aucMac);

    if (NULL == pstEntry)
    {
        pstEntry = nf_portal_add_sta_to_htable(pstIoStaType->aucMac,pstIoStaType->uiIp);
   
        if (unlikely(NULL == pstEntry))
        {
            NF_PORTAL_LOG(NF_PORTAL_LOG_ERR,"["MAC_FMT"] not exist,return\n",MAC_ARG(pstIoStaType->aucMac));
            return AP_E_PARAM;
        } 
    }
	else{
		if(3 == pstIoStaType->enStaType)
			nf_portal_del_sta_from_htable(pstEntry);
	}
    
    NF_PORTAL_LOG(NF_PORTAL_LOG_INFO,"["MAC_FMT"]:%d\n",MAC_ARG(pstIoStaType->aucMac),pstIoStaType->enStaType);
    return nf_portal_sta_type_update(pstEntry,pstIoStaType->enStaType);
}
...
AP_ERROR_CODE_E nf_portal_ioctl_get_sta_type(NF_PORTAL_IOCTL_STA_TYPE_T *pstIoStaType)
{
    ...
    if (unlikely(NULL == pstEntry))
    {
        NF_PORTAL_LOG(NF_PORTAL_LOG_INFO,"["MAC_FMT",] not exist,return\n",MAC_ARG(pstIoStaType->aucMac));
        return AP_E_PARAM;
    }
    
    pstIoStaType->enStaType = nf_portal_get_sta_type(pstEntry);
    return AP_E_NONE;
}
AP_ERROR_CODE_E nf_portal_ioctl_get_net_model(NF_PORTAL_IOCTL_NET_MODEL_T *pstIoNetModel)
{
    if (unlikely(NULL == pstIoNetModel))
    {
        return AP_E_PARAM;
    }
    pstIoNetModel->enNetModel = nf_portal_get_net_model();
    return AP_E_NONE;
}
...
AP_ERROR_CODE_E nf_portal_ioctl_set_net_model(NF_PORTAL_IOCTL_NET_MODEL_T *pstIoNetModel)
{
    ...
    NF_PORTAL_LOG(NF_PORTAL_LOG_INFO,"NetModel:%u\n",pstIoNetModel->enNetModel);
    return  nf_portal_set_net_model(pstIoNetModel->enNetModel);
}
...
AP_ERROR_CODE_E nf_portal_ioctl_get_lan_free_auth_sw(NF_PORTAL_IOCTL_SW_T *pstIoSW)
{
    ...
    pstIoSW->enSW = nf_portal_get_lan_free_auth_sw();
    return AP_E_NONE;
}
...
AP_ERROR_CODE_E nf_portal_ioctl_set_lan_free_auth_sw(NF_PORTAL_IOCTL_SW_T *pstIoSW)
{
    ...
    NF_PORTAL_LOG(NF_PORTAL_LOG_INFO,"sw:%u\n",pstIoSW->enSW);
    return  nf_portal_set_lan_free_auth_sw(pstIoSW->enSW);
}
/*end:lan_free_auth 2015-9-6*/

/*begin:net connect 2015-9-18*/
AP_ERROR_CODE_E nf_portal_ioctl_get_net_connect(NF_PORTAL_IOCTL_SW_T *pstIoSW)
{
    ...
    pstIoSW->enSW = nf_portal_get_net_connect();
    return AP_E_NONE;
}
...
AP_ERROR_CODE_E nf_portal_ioctl_set_net_connect(NF_PORTAL_IOCTL_SW_T *pstIoSW)
{
    ...
    NF_PORTAL_LOG(NF_PORTAL_LOG_INFO,"sw:%u\n",pstIoSW->enSW);
    return  nf_portal_set_net_connect(pstIoSW->enSW);
}
/*end:net connect 2015-9-6*/
...
AP_ERROR_CODE_E nf_portal_ioctl_dest_ip_baw_add(NF_PORTAL_IOCTL_IP_BAW_T *pstIoIpBaw)
{
	...
	if(16843009 == pstIoIpBaw->stIpBaw.uiIp) /*1.1.1.1的ip有特殊用途，不得加入黑白名单*/
		return AP_E_PARAM;
...
{
    if (unlikely(NULL == pstIoNetBaw))
    {
        return AP_E_PARAM;
    }
    
    NF_PORTAL_LOG(NF_PORTAL_LOG_INFO,"ip:"IP_FMT"/%u,flag:%u\n",IP_ARG(&(pstIoNetBaw->stNetBaw.uiNet)),\
        pstIoNetBaw->stNetBaw.uiMaskNum,pstIoNetBaw->stNetBaw.enFlag);
    printk("ip:"IP_FMT"/%u,flag:%u\n",IP_ARG(&(pstIoNetBaw->stNetBaw.uiNet)),\
        pstIoNetBaw->stNetBaw.uiMaskNum,pstIoNetBaw->stNetBaw.enFlag);


    return nf_portal_dest_net_baw_add_tomap(pstIoNetBaw->stNetBaw.uiNet,pstIoNetBaw->stNetBaw.uiMaskNum,pstIoNetBaw->stNetBaw.enFlag);
}
...
AP_ERROR_CODE_E nf_portal_ioctl_dest_net_baw_del(NF_PORTAL_IOCTL_NET_BAW_T *pstIoNetBaw)
{
    ...
    printk("ip:"IP_FMT"/%u,flag:%u\n",IP_ARG(&(pstIoNetBaw->stNetBaw.uiNet)),\
        pstIoNetBaw->stNetBaw.uiMaskNum,pstIoNetBaw->stNetBaw.enFlag);
    if(0 == pstIoNetBaw->stNetBaw.uiMaskNum)
    {
        for (uiMaskNum = 0;uiMaskNum < NET_MASK_NUM_MAX; uiMaskNum++)
        {
            if(AP_E_NONE == nf_portal_dest_net_baw_del_frommap(pstIoNetBaw->stNetBaw.uiNet,uiMaskNum,pstIoNetBaw->stNetBaw.enFlag))
            {
                return AP_E_NONE;
            }
        }
    ...
    return nf_portal_dest_net_baw_del_frommap(pstIoNetBaw->stNetBaw.uiNet,pstIoNetBaw->stNetBaw.uiMaskNum,pstIoNetBaw->stNetBaw.enFlag);
}

AP_ERROR_CODE_E nf_portal_ioctl_dest_net_baw_get_list(NF_PORTAL_IOCTL_NET_BAW_LIST_T *pstIoNetBawList)
{
    if (unlikely(NULL == pstIoNetBawList))
    {
        return AP_E_PARAM;
    }
    
    NF_PORTAL_LOG(NF_PORTAL_LOG_INFO,"start num:%u,num:%u,flag:%u\n",pstIoNetBawList->uiStartNum,\
        pstIoNetBawList->uiNum,pstIoNetBawList->enFlag);
    return nf_portal_dest_net_baw_get_list(pstIoNetBawList);
}
...
AP_ERROR_CODE_E nf_portal_ioctl_dest_net_baw_flush(NF_PORTAL_IOCTL_NET_BAW_T *pstIoNetBaw)
{
    ...
    NF_PORTAL_LOG(NF_PORTAL_LOG_INFO,"flag:%u\n",pstIoNetBaw->stNetBaw.enFlag);
    return nf_portal_dest_net_baw_flush(pstIoNetBaw->stNetBaw.enFlag);
}
...
AP_ERROR_CODE_E nf_portal_ioctl_url_ip_baw_add(NF_PROTAL_IOCTL_URL_IP_BAW_T *pstIoUrlIpBaw)
{
	...
	if(16843009 == pstIoUrlIpBaw->uiIp) /*1.1.1.1的ip有特殊用途，不得加入白名单*/
		return AP_E_PARAM;
...
{
    if (unlikely(NULL == pstIoWanCfg))
    {
        return AP_E_PARAM;
    }

    return nf_portal_wan_conf_set(pstIoWanCfg->uiWanIndex, pstIoWanCfg->uiEnable, pstIoWanCfg->uiBand);
}
...
AP_ERROR_CODE_E nf_portal_ioctl_wan_conf_show(NF_PORTAL_IOCTL_WAN_LIST_T *pstIoWanList)
{
    ...
    return nf_portal_wan_conf_get(pstIoWanList);
}
...
AP_ERROR_CODE_E nf_portal_ioctl_wan_map_show(NF_PORTAL_IOCTL_WAN_MAP_T *pstIoWanMap)
{
    ...
    return nf_portal_wan_map_get(pstIoWanMap);    
}
#endif
...
{
    if (unlikely(NULL == pstIoVlanFlood))
    {
        return AP_E_PARAM;
    }
    
    NF_PORTAL_LOG(NF_PORTAL_LOG_INFO,"vlan_flood: %s\n", pstIoVlanFlood->bEnable ? "enable" : "disable");
    return nf_portal_set_vlan_flood(pstIoVlanFlood->bEnable);
}
...
AP_ERROR_CODE_E nf_portal_ioctl_get_vlan_flood(NF_PORTAL_IOCTL_VLAN_FLOOD_T *pstIoVlanFlood)
{
    ...
    pstIoVlanFlood->bEnable = nf_portal_get_vlan_flood();
    return AP_E_NONE;
}
