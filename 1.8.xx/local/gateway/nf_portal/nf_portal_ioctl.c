...
AP_ERROR_CODE_E nf_portal_ioctl_handler(void *pstArg)
{
	...
	switch (stIoctl.enCmd)
	{
		...
		case NF_PORTAL_IOCTL_CMD_GET_AUTH_STATE:
			...
			bGetOps = TRUE;
			break;
        /*begin: lan_free_auth  2015-8-18*/
        case NF_PORTAL_IOCTL_CMD_GET_LAN_FREE_AUTH_SW:
        {
            enRet = nf_portal_ioctl_get_lan_free_auth_sw(&stIoctl.unData.stIoSW);
            bGetOps = TRUE;
            break;
        }
        case NF_PORTAL_IOCTL_CMD_SET_LAN_FREE_AUTH_SW:
        {
            enRet = nf_portal_ioctl_set_lan_free_auth_sw(&stIoctl.unData.stIoSW);
            break;
        }
        /*end: lan_free_auth 2015-8-18*/
        /*begin: net connect  2015-9-18*/
        case NF_PORTAL_IOCTL_CMD_GET_NET_CONNECT:
        {
            enRet = nf_portal_ioctl_get_net_connect(&stIoctl.unData.stIoSW);
            bGetOps = TRUE;
            break;
        }
        case NF_PORTAL_IOCTL_CMD_SET_NET_CONNECT:
        {
            enRet = nf_portal_ioctl_set_net_connect(&stIoctl.unData.stIoSW);
            break;
        }
        /*end: net connect 2015-9-18*/
        ...
        case NF_PORTAL_IOCTL_CMD_DEST_IP_BAW_FLUSH:
            ...
            break;
        case NF_PORTAL_IOCTL_CMD_ADD_DEST_NET_BAW:
            enRet = nf_portal_ioctl_dest_net_baw_add(&stIoctl.unData.stIoNetBaw);
            break;
        case NF_PORTAL_IOCTL_CMD_DEL_DEST_NET_BAW:
            enRet = nf_portal_ioctl_dest_net_baw_del(&stIoctl.unData.stIoNetBaw);
            break;
        case NF_PORTAL_IOCTL_CMD_GET_DEST_NET_BAW_LIST:
            enRet = nf_portal_ioctl_dest_net_baw_get_list(&stIoctl.unData.stIoNetBawList);
            bGetOps = TRUE;
            break;
        case NF_PORTAL_IOCTL_CMD_DEST_NET_BAW_FLUSH:
            enRet = nf_portal_ioctl_dest_net_baw_flush(&stIoctl.unData.stIoNetBaw);
            break;
		...
		/*end: add@2015.01.31 for wechat connect wifi*/
 	    case NF_PORTAL_IOCTL_CMD_SET_STA_TYPE:
			enRet = nf_portal_ioctl_set_sta_type(&stIoctl.unData.stIoStaType);
			break;
		case NF_PORTAL_IOCTL_CMD_GET_STA_TYPE:
			enRet = nf_portal_ioctl_get_sta_type(&stIoctl.unData.stIoStaType);
			bGetOps = TRUE;
			break;
 	    case NF_PORTAL_IOCTL_CMD_SET_NET_MODEL:
			enRet = nf_portal_ioctl_set_net_model(&stIoctl.unData.stIoNetModel);
			break;
		case NF_PORTAL_IOCTL_CMD_GET_NET_MODEL:
			enRet = nf_portal_ioctl_get_net_model(&stIoctl.unData.stIoNetModel);
			bGetOps = TRUE;
			break;

#ifdef HAVE_MULTI_WAN
		case NF_PORTAL_IOCTL_CMD_WAN_CONFIG_SET:
			enRet = nf_portal_ioctl_wan_conf_set(&stIoctl.unData.stIoWanCfg);
			break;
		case NF_PORTAL_IOCTL_CMD_WAN_LIST_GET:
			enRet = nf_portal_ioctl_wan_conf_show(&stIoctl.unData.stIoWanList);
			bGetOps = TRUE;
			break;
		case NF_PORTAL_IOCTL_CMD_WAN_MAP_GET:
			enRet = nf_portal_ioctl_wan_map_show(&stIoctl.unData.stIoWanMap);
			bGetOps = TRUE;
			break;
#endif
		case NF_PORTAL_IOCTL_CMD_SET_VLAN_FLOOD:
			enRet = nf_portal_ioctl_set_vlan_flood(&stIoctl.unData.stIoVlanFlood);
			break;
		case NF_PORTAL_IOCTL_CMD_GET_VLAN_FLOOD:
		    enRet = nf_portal_ioctl_get_vlan_flood(&stIoctl.unData.stIoVlanFlood);
		    bGetOps = TRUE;
		    break;
        default:
