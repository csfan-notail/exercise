...
#define LEASES_FILE		"/dev/shm/dhcpd.leases.awkfile"
#define LEASES_FILE_TAC		"/dev/shm/dhcpd.leases.awkfile.tac"
#define LEASES_FILE_RAM "/dev/shm/dhcpd.leases"
#else
#define LEASES_FILE		"/var/lib/misc/dhcp_dnsmasq.leases"
#endif
...
/* BEGIN: Added by zoucaihong, 2015/3/19 */
BOOL get_sta_info_from_dhcp(AP_STAT_STATION_ENTRY_T *pstUserInfo)
{
    CHAR *pos = NULL;
    CHAR *pos2 = NULL;
    CHAR *pos3 = NULL;
    ...
    char acUdhcpPid[32] = "";
#if (defined(GW_X86) || defined(GW_ARM) || defined(GW_AC))
    char *temp = (char *)malloc(5*1024*1024);   //5M
    memset(temp, 0, 5*1024*1024);
#else
    char *temp = (char *)malloc(2*1024*1024);     //2M
    memset(temp, 0, 2*1024*1024);
#endif

    if ( !(fp = fopen(LEASES_FILE, "r")) )
    {
        agent_printf(LOG_ERR, "fopen "LEASES_FILE" fail.\n");
        free(temp);
        return FALSE;
    }
    fseek(fp,0,SEEK_END);
    ...
    fread(temp, ulLen, 1, fp);
    fclose(fp);
    
    sprintf(acMac, MAC_FMT, MAC_ARG(pstUserInfo->aucStaMac));
    
    if ( NULL != (pos = strstr(temp, acMac)) )
    {
        /* IP */
        pos2 = strchr(pos + 18, ' ');
        strncpy(acIp, pos + 18, pos2 - pos - 18);
        string_ip_tools(acIp, pstUserInfo->aucStaIp);
        /* hostname */
        pos3 = strchr(pos2 + 1, ' ');
        strncpy(pstUserInfo->aucStaDevType, pos2 + 1, pos3 - pos2 - 1);
    }
    free(temp);
    return TRUE;
}
/* END: Added by zoucaihong, 2015/3/19 */
#if 0
/*****************************************************************************
...
BOOL wlan_get_userinfo(AP_STAT_STATION_ENTRY_T *pstUserInfo, UINT32 *pulUserNum)
{
    ...
    {      
        ...
        /* 获取VLAN ID */
        sprintf(acNvramName, NVRAM_VAP"%d_vlan_list", vaps_list[iLoop]);
        nvram_get(acVlanId, acNvramName, 32);
        ulVlanId = atoi(acVlanId);
        /* 获取VLAN是否启用 */
        sprintf(acNvramName, NVRAM_VAP"%d_tag", vaps_list[iLoop]);
        ...
        else
        {
            ...
            for(loop = 0 ; loop < ulMaxEndUser ; loop ++)
                { 
                    ...
                    //wlan_get_stadevtype( pstUserInfo[iCount].aucStaMac, pstUserInfo[iCount].aucStaDevType );
                    /* BEGIN: Added by zoucaihong, 2015/3/19 */
                    get_sta_info_from_dhcp(&pstUserInfo[iCount]);
                    /* END: Added by zoucaihong, 2015/3/19 */
                    pstUserInfo[iCount].ulConnectTime = stEndUserOmc.pstStaStats[loop].ulStaLiveTime;
                    pstUserInfo[iCount].ulStaUplinkOctets = stEndUserOmc.pstStaStats[loop].ullTxDataBytes;
                    pstUserInfo[iCount].ulStaDownlinkOctets = stEndUserOmc.pstStaStats[loop].ullRxDataBytes;
