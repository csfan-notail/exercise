...
AP_ERROR_CODE_E fc_ioctl_dest_ip_add(FC_IOCTL_IP_T *pstIp)
{
    if (unlikely(NULL == pstIp))
    {
        return AP_E_PARAM;
    }
    
    FC_LOG(FC_LOG_INFO,"ip:"IP_FMT"\n",IP_ARG(&(pstIp->uiIp)));
    return fc_dest_ip_add(pstIp->uiIp);
}

AP_ERROR_CODE_E fc_ioctl_dest_ip_del(FC_IOCTL_IP_T *pstIp)
{
    ...
    pstEntry = fc_dest_ip_get(pstIp->uiIp);
    if (NULL == pstEntry)
    {
        FC_LOG(FC_LOG_ERR,"ip:"IP_FMT"\n",IP_ARG(&(pstIp->uiIp)));
        return AP_E_NOT_EXIST;
    }
    
    FC_LOG(FC_LOG_INFO,"ip:"IP_FMT"\n",IP_ARG(&(pstIp->uiIp)));
    return fc_dest_ip_del(pstEntry);
}

AP_ERROR_CODE_E fc_ioctl_dest_ip_get_list(FC_IOCTL_IP_LIST_T *pstIpList)
{
    if (unlikely(NULL == pstIpList))
    {
        return AP_E_PARAM;
    }
    
    FC_LOG(FC_LOG_INFO,"start num:%u,num:%u\n",pstIpList->uiStartNum,pstIpList->uiNum);
    return fc_dest_ip_get_list(pstIpList);
}

AP_ERROR_CODE_E fc_ioctl_dest_ip_flush(void)
{  
    return fc_dest_ip_flush();
}
...
AP_ERROR_CODE_E fc_ioctl_handler(void *pstArg)
{
	...
	switch (stIoctl.enCmd)
	{
        ...
        case FC_IOCTL_CMD_GET_DEV_SSID_LIST: 
            ...
            bGetOper = TRUE;
            break;
        case FC_IOCTL_CMD_ADD_IP:
            enRet = fc_ioctl_dest_ip_add(&stIoctl.u.stIp);
            break;
        case FC_IOCTL_CMD_DEL_IP:
            enRet =fc_ioctl_dest_ip_del(&stIoctl.u.stIp);
            break;
        case FC_IOCTL_CMD_GET_IP_LIST:
            enRet = fc_ioctl_dest_ip_get_list(&stIoctl.u.stIpList);
            bGetOper = TRUE;
            break;
        case FC_IOCTL_CMD_FLUSH_IP_LIST:
            enRet = fc_ioctl_dest_ip_flush();
            break;
        default:
