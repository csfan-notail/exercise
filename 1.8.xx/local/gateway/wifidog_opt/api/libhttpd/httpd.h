...
#define HTTP_OUTPUT_FLAG_COMMON			0
#define HTTP_OUTPUT_FLAG_APPLE			1
#define HTTP_OUTPUT_FLAG_AJAX			2
#define HTTP_OUTPUT_FLAG_FIRST_PAGE		3
#define HTTP_OUTPUT_FLAG_200_OK			4


extern char 	LIBHTTPD_VERSION[],
		LIBHTTPD_VENDOR[];

#define URL_STANDAR_CODE_FREE(x) {if(NULL != (x)) {free((x));}}
...
typedef	struct {
    ...
    char userAgent[HTTP_MAX_URL]; /*add@2014.10.13*/
#if(0)
...
#define READ_FD_SET     1
#define NOT_READ_FD_SET 0
...
typedef struct {
	int	clientSock,
		...
		readBufRemain;
    int readFlag;
    int freeFlag; /** @在收到/wifi/auth?流程中不需要free这个结构体 */
    //time_t clientSock_timeout; /** @增加socket TCP 超时机制 */
    //time_t authSock_timeout; /** @增加socket TCP 超时机制 */
    long clientSock_timeout; /** @增加socket TCP 超时机制 */
    long authSock_timeout; /** @增加socket TCP 超时机制 */
	httpReq	request;
	httpRes response;
	httpVar	*variables;
...
void httpdSendContentBody __ANSI_PROTO((request *,char *,int ));
void  httpdSendContentBodyAjax __ANSI_PROTO((request *,char *,int ));
...
char *http_url_standard_decode __ANSI_PROTO((const char *,const int ));
char *http_url_standard_encode __ANSI_PROTO((const char *,const int ));
...
char *http_parse_name_of_value __ANSI_PROTO((const char *,const char *));
