...
# hide config menu if local source path was defined
ifneq ($(PKG_SOURCE_LOCAL),)
define Package/$(PKG_NAME)/override_version
endef
define Package/$(PKG_NAME)/override_source_path
endef
CONFIG_$(PKG_NAME)_USE_CUSTOM_SOURCE_DIR=y
CONFIG_$(PKG_NAME)_CUSTOM_SOURCE_DIR=$(PKG_SOURCE_LOCAL)
endif

define Build/Clean
	-$(call Build/Compile/Default,clean)
	-cd $(PKG_BUILD_DIR) && rm -fr ipkg-ar71xx .built*  .configured* .prepared* .dep_files
endef
